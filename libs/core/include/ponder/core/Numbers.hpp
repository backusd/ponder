...
template <typename T>
concept RoundableIntegral = NonBoolIntegral<T> && std::numeric_limits<std::remove_cvref_t<T>>::digits <= std::numeric_limits<long double>::digits;
...
[[nodiscard]] std::optional<std::remove_cvref_t<Target>> RoundToInteger(Source value) noexcept
{
    ...
    constexpr long double kLowest = static_cast<long double>(std::numeric_limits<TargetValue>::lowest());
    constexpr long double kHighest = static_cast<long double>(std::numeric_limits<TargetValue>::max());
...
{
public:
    [[nodiscard]] static constexpr Tolerance Create(float absoluteTolerance, float relativeTolerance)
    {
        if (!IsFinite(absoluteTolerance) || !IsFinite(relativeTolerance)) [[unlikely]]
        {
            throw PONDER_EXCEPTION("Tolerance values must be finite.");
        }

        if (absoluteTolerance < 0.0F || relativeTolerance < 0.0F) [[unlikely]]
        {
            throw PONDER_EXCEPTION("Tolerance values must be non-negative.");
        }
    ...
    [[nodiscard]] friend constexpr bool operator==(const Tolerance& lhs, const Tolerance& rhs) noexcept = default;

private:
    explicit constexpr Tolerance(float absoluteTolerance, float relativeTolerance) noexcept :
        m_absolute(absoluteTolerance),
        m_relative(relativeTolerance)
    {
    }
...
[[nodiscard]] constexpr bool IsNear(float lhs, float rhs, Tolerance tolerance) noexcept
{
    ...
    const double largestInput = std::max(detail::AbsoluteValue(lhsWide), detail::AbsoluteValue(rhsWide));
