...
concept ScopeExitCallback =
    std::is_nothrow_move_constructible_v<Callback> && std::is_nothrow_destructible_v<Callback> && std::is_nothrow_invocable_v<Callback&>;

template <ScopeExitCallback Callback>
class [[nodiscard]] ScopeExit final
{
public:
    explicit constexpr ScopeExit(Callback callback) noexcept :
...
template <typename Callback>
    requires ScopeExitCallback<std::decay_t<Callback>> && std::is_nothrow_constructible_v<std::decay_t<Callback>, Callback&&>
