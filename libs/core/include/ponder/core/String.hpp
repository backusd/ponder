...
constexpr void AppendWideCodePoint(std::wstring& output, std::uint32_t codePoint)
{
...
constexpr void AppendUtf8CodePoint(std::string& output, std::uint32_t codePoint)
{
    if (codePoint <= 0x7FU)
    {
...
[[nodiscard]] constexpr Result<std::wstring> Utf8ToWideString(std::string_view text)
{
    ...
    while (index < text.size())
    {
        ...
        {
            return detail::MakeInvalidUtf8Error();
        }

        detail::AppendWideCodePoint(output, codePoint);
    }

    return output;
}

[[nodiscard]] constexpr Result<std::string> WideStringToUtf8(std::wstring_view text)
{
    std::string output;
    output.reserve(text.size());
    ...
    {
        std::uint32_t codePoint{};
        if (!detail::TryReadWideCodePoint(text, index, codePoint))
        {
            return detail::MakeInvalidWideStringError();
        }

        detail::AppendUtf8CodePoint(output, codePoint);
    }
