...
[[nodiscard]] std::optional<DisplayId> ResolveDisplayId(
    void* context, std::uint32_t backendDisplayId)
{
    auto& routing = *static_cast<RuntimeEventTranslationContext*>(context);
    if (routing.removedDisplayId.has_value() &&
        routing.removedBackendDisplayId == backendDisplayId)
    {
...
void PlatformRuntimeState::ObserveWindowShownEvent(
    std::uint32_t backendWindowId)
{
    ...
    if (window != m_windowsByBackendId.end())
    {
        PONDER_VERIFY(window->second.window != nullptr,
                      "Registered backend window {} has no owning window",
                      backendWindowId);
...
std::optional<PlatformEvent> PlatformRuntimeState::PollEvent()
{
    ...
    while (true)
    {
        ...
        {
            static_cast<void>(ConnectDisplayFromEvent(
                static_cast<std::uint32_t>(event.display.displayID)));
        }
        else if (event.type == SDL_EVENT_DISPLAY_REMOVED)
        {
            routing.removedBackendDisplayId =
                static_cast<std::uint32_t>(event.display.displayID);
            routing.removedDisplayId =
                FindDisplayIdForRemoval(routing.removedBackendDisplayId);
        }
        ...
        {
            ObserveWindowShownEvent(
                static_cast<std::uint32_t>(event.window.windowID));
        }

        const EventTranslationContext translationContext{
            &routing, ResolveWindowId, ResolveDisplayId};
        std::optional<PlatformEvent> translated =
            TranslateSdlEvent(event, translationContext);

        if (event.type == SDL_EVENT_DISPLAY_REMOVED &&
            routing.removedDisplayId.has_value())
        {
