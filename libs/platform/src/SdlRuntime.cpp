...
namespace
{
constexpr std::string_view kHintLogCategory{"platform"};
...
[[nodiscard]] bool HintMatchesValue(const char* name, const std::optional<std::string>& value) noexcept
{
...
void HintValidateMutationPhase(const char* name, bool beforeInitializationOnly)
{
    if (beforeInitializationOnly && SDL_WasInit(0) != 0)
    {
...
void SdlRuntime::HintPushRaw(const char* name, bool beforeInitializationOnly, bool pushOnce, std::string value)
{
    ...
    HintValidateMutationPhase(name, beforeInitializationOnly);
    ...
    if (pushOnce && currentValue != nullptr)
    {
        throw PLATFORM_EXCEPTION(PlatformErrorCode::InvalidArgument, "{} can only be set once and is already set.", name);
    }
...
void SdlRuntime::HintPopRaw(const char* name, bool beforeInitializationOnly)
{
...
void SdlRuntime::HintClearRaw(const char* name, bool beforeInitializationOnly)
{
...
void SdlRuntime::HintRestoreAll() noexcept
{
    ...
    for (auto iterator = m_activeHintNames.rbegin(); iterator != m_activeHintNames.rend(); ++iterator)
    {
        try
        {
            ...
            {
                LOG_ERROR_CATEGORY(kHintLogCategory, "Platform hint restoration bookkeeping is inconsistent");
                continue;
            }
        ...
        {
            LOG_ERROR_CATEGORY(kHintLogCategory, "Platform hint restoration failed: {}", exception.GetMessage());
        }
        catch (const std::exception& exception)
        {
            LOG_ERROR_CATEGORY(kHintLogCategory, "Platform hint restoration failed: {}", exception.what());
        }
        catch (...)
        {
            LOG_ERROR_CATEGORY(kHintLogCategory, "Platform hint restoration failed");
        }
    }
...
#define PONDER_DEFINE_HINT_POP_AND_CLEAR(Type, Name, BeforeInitializationOnly)                                                                       \
    template <>                                                                                                                                      \
    void SdlRuntime::HintPop<hints::Type>()                                                                                                          \
    {                                                                                                                                                \
        HintPopRaw(Name, BeforeInitializationOnly);                                                                                                  \
    }                                                                                                                                                \
                                                                                                                                                     \
    template <>                                                                                                                                      \
    void SdlRuntime::HintClear<hints::Type>()                                                                                                        \
    {                                                                                                                                                \
        HintClearRaw(Name, BeforeInitializationOnly);                                                                                                \
    }

#define PONDER_DEFINE_BOOLEAN_HINT(Type, Name, BeforeInitializationOnly, PushOnce)                                                                   \
    template <>                                                                                                                                      \
    void SdlRuntime::HintPush<hints::Type>(const hints::Type& hint)                                                                                  \
    {                                                                                                                                                \
        HintPushRaw(Name, BeforeInitializationOnly, PushOnce, std::format("{}", hint));                                                              \
    }                                                                                                                                                \
                                                                                                                                                     \
    PONDER_DEFINE_HINT_POP_AND_CLEAR(Type, Name, BeforeInitializationOnly)                                                                           \
                                                                                                                                                     \
    template <>                                                                                                                                      \
    ...
    {                                                                                                                                                \
        ...
        if (value.empty())                                                                                                                           \
        {                                                                                                                                            \
            LOG_ERROR_CATEGORY(kHintLogCategory, "SDL returned an empty boolean value for platform hint {}", Name);                                  \
            return std::nullopt;                                                                                                                     \
        }                                                                                                                                            \
...
std::optional<hints::EventLogging> SdlRuntime::HintGet<hints::EventLogging>() const
{
    ...
    LOG_ERROR_CATEGORY(kHintLogCategory, "SDL returned invalid value '{}' for platform hint {}", *value, SDL_HINT_EVENT_LOGGING);
    return std::nullopt;
}
...
std::optional<hints::ImeImplementedUi> SdlRuntime::HintGet<hints::ImeImplementedUi>() const
{
    ...
    LOG_ERROR_CATEGORY(kHintLogCategory, "SDL returned invalid value '{}' for platform hint {}", *value, SDL_HINT_IME_IMPLEMENTED_UI);
    return std::nullopt;
}

#define PONDER_DEFINE_STRING_HINT(Type, Name, BeforeInitializationOnly)                                                                              \
    template <>                                                                                                                                      \
    void SdlRuntime::HintPush<hints::Type>(const hints::Type& hint)                                                                                  \
    {                                                                                                                                                \
        ...
        HintPushRaw(Name, BeforeInitializationOnly, false, std::format("{}", hint));                                                                 \
    }                                                                                                                                                \
    PONDER_DEFINE_HINT_POP_AND_CLEAR(Type, Name, BeforeInitializationOnly);                                                                          \
    template <>                                                                                                                                      \
    std::optional<hints::Type> SdlRuntime::HintGet<hints::Type>() const                                                                              \
    {                                                                                                                                                \
        ...
        if (value->empty())                                                                                                                          \
        {                                                                                                                                            \
            LOG_ERROR_CATEGORY(kHintLogCategory, "SDL returned an empty string value for platform hint {}", Name);                                   \
            return std::nullopt;                                                                                                                     \
        }                                                                                                                                            \
...
std::optional<hints::VideoMinimizeOnFocusLoss> SdlRuntime::HintGet<hints::VideoMinimizeOnFocusLoss>() const
{
    ...
    LOG_ERROR_CATEGORY(kHintLogCategory, "SDL returned invalid value '{}' for platform hint {}", *value, SDL_HINT_VIDEO_MINIMIZE_ON_FOCUS_LOSS);
    return std::nullopt;
}
...
std::optional<hints::MouseDefaultSystemCursor> SdlRuntime::HintGet<hints::MouseDefaultSystemCursor>() const
{
    ...
    if (!parsed.has_value())
    {
        LOG_ERROR_CATEGORY(kHintLogCategory, "SDL returned invalid value '{}' for platform hint {}", *value, SDL_HINT_MOUSE_DEFAULT_SYSTEM_CURSOR);
        return std::nullopt;
    }
    ...
    if (!cursor.has_value())
    {
        LOG_ERROR_CATEGORY(kHintLogCategory, "SDL returned unsupported value '{}' for platform hint {}", *value,
...
std::optional<hints::MouseDoubleClickRadius> SdlRuntime::HintGet<hints::MouseDoubleClickRadius>() const
{
    ...
    {
        LOG_ERROR_CATEGORY(kHintLogCategory, "SDL returned invalid value '{}' for platform hint {}", *value, SDL_HINT_MOUSE_DOUBLE_CLICK_RADIUS);
        return std::nullopt;
    }
...
std::optional<hints::MouseDoubleClickTime> SdlRuntime::HintGet<hints::MouseDoubleClickTime>() const
{
    ...
    {
        LOG_ERROR_CATEGORY(kHintLogCategory, "SDL returned invalid value '{}' for platform hint {}", *value, SDL_HINT_MOUSE_DOUBLE_CLICK_TIME);
        return std::nullopt;
    }
...
#define PONDER_DEFINE_FLOAT_HINT(Type, Name)                                                                                                         \
    ...
    {                                                                                                                                                \
        ...
        {                                                                                                                                            \
            LOG_ERROR_CATEGORY(kHintLogCategory, "SDL returned invalid value '{}' for platform hint {}", *value, Name);                              \
            return std::nullopt;                                                                                                                     \
        }                                                                                                                                            \
...
ponder::core::Result<std::string> SdlRuntime::ClipboardGetText() const
{
    ...
    if (rawText == nullptr || (rawText[0] == '\0' && rawError != nullptr))
    {
...
[[nodiscard]] constexpr bool IsValidFilterPattern(std::string_view pattern) noexcept
{
    if (pattern == "*")
    {
        return true;
...
void ValidateFilter(const dialogs::DialogFileFilter& filter, std::size_t index)
{
...
struct PreparedDialogRequest final
{
    ...
    std::vector<dialogs::DialogFileFilter> filters;
    bool allowMultipleSelection{};
};

[[nodiscard]] PreparedDialogRequest PrepareDialogRequest(dialogs::DialogKind kind, std::optional<WindowId> parentWindowId,
...
[[nodiscard]] constexpr std::string_view GetDialogOperation(dialogs::DialogKind kind) noexcept
{
    switch (kind)
    {
    case dialogs::DialogKind::OpenFile:
        return "SDL_ShowOpenFileDialog";
    case dialogs::DialogKind::SaveFile:
        return "SDL_ShowSaveFileDialog";
    case dialogs::DialogKind::OpenFolder:
...
[[nodiscard]] DialogFailure MakeDialogFailure(dialogs::DialogKind kind, std::string_view message)
{
...
[[nodiscard]] ponder::core::Error MakeDialogApiError(std::string_view operation, std::exception_ptr exceptionPointer) noexcept
{
    try
    {
    ...
    {
        LOG_ERROR_CATEGORY(kLogCategory, "{} failed with a platform exception: {}", operation, exception.GetMessage());
        return ponder::core::Error{kBackendFailureCode, std::format("{} failed: {}", operation, exception.GetMessage()), exception.GetStackTrace(),
                                   exception.GetLocation()};
    }
    catch (const std::exception& exception)
    {
        LOG_ERROR_CATEGORY(kLogCategory, "{} failed with a standard exception: {}", operation, exception.what());
        return ponder::core::Error{kBackendFailureCode, std::format("{} failed with a standard exception: {}", operation, exception.what())};
    }
    catch (...)
    {
        LOG_ERROR_CATEGORY(kLogCategory, "{} failed with an unknown exception", operation);
        return ponder::core::Error{kBackendFailureCode, std::format("{} failed with an unknown exception.", operation)};
    }
}
} // namespace
...
public:
    ...
    void EnqueueCompletion(dialogs::DialogRequestId id, ponder::core::Timestamp timestamp, DialogOutcome outcome)
    {
    ...
    void MarkCallbackFailure(dialogs::DialogRequestId id, ponder::core::Timestamp timestamp)
    {
        std::scoped_lock lock{m_mutex};
        if (m_runtime != nullptr)
        {
    ...
    void Deactivate(SdlRuntime& runtime)
    {
        std::scoped_lock lock{m_mutex};
...
public:
    ...
    DialogCompletionRecord completion;
    std::list<dialogs::DialogRequestId> completionQueueNode;
...
{
public:
    SdlDialogContext(std::weak_ptr<DialogCallbackHandoff> callbackHandoff, dialogs::DialogRequestId requestId, dialogs::DialogKind kind,
                     bool allowMultipleSelection, std::optional<std::string> defaultLocation, std::vector<dialogs::DialogFileFilter> filters) :
        callbackHandoff(std::move(callbackHandoff)),
        requestId(requestId),
        kind(kind),
        allowMultipleSelection(allowMultipleSelection),
        defaultLocation(std::move(defaultLocation)),
        filters(std::move(filters))
    {
        nativeFilters.reserve(filters.size());
        for (const dialogs::DialogFileFilter& filter : filters)
        {
            nativeFilters.push_back(SDL_DialogFileFilter{.name = filter.name.c_str(), .pattern = filter.pattern.c_str()});
        }
    }

    void Complete(ponder::core::Timestamp timestamp, DialogOutcome outcome) const
    {
        if (const std::shared_ptr<DialogCallbackHandoff> handoff = callbackHandoff.lock())
        {
            handoff->EnqueueCompletion(requestId, timestamp, std::move(outcome));
        }
    }

    void MarkCallbackFailure(ponder::core::Timestamp timestamp) const noexcept
    {
        try
        {
            if (const std::shared_ptr<DialogCallbackHandoff> handoff = callbackHandoff.lock())
            {
                handoff->MarkCallbackFailure(requestId, timestamp);
            }
        }
        catch (const std::exception& exception)
        {
            LOG_ERROR_CATEGORY(kLogCategory, "Could not enqueue the fallback failure for dialog request {}: {}", requestId, exception.what());
        }
        catch (...)
        {
            LOG_ERROR_CATEGORY(kLogCategory, "Could not enqueue the fallback failure for dialog request {} due to an unknown exception", requestId);
        }
    }

    std::weak_ptr<DialogCallbackHandoff> callbackHandoff;
    dialogs::DialogRequestId requestId;
    dialogs::DialogKind kind{dialogs::DialogKind::OpenFile};
    bool allowMultipleSelection{};
    std::optional<std::string> defaultLocation;
    std::vector<dialogs::DialogFileFilter> filters;
...
[[nodiscard]] DialogOutcome MakeDialogSelection(const SdlDialogContext& context, const char* const* fileList, int selectedFilter)
{
    if (!context.allowMultipleSelection && fileList[1] != nullptr)
    {
        return MakeDialogFailure(context.kind, "SDL returned multiple paths for a single-selection dialog request.");
    }

    std::vector<std::filesystem::path> paths;
    for (std::size_t index = 0; fileList[index] != nullptr; ++index)
    {
        const std::string_view pathText{fileList[index]};
        if (pathText.empty())
        {
            return MakeDialogFailure(context.kind, "SDL returned an empty dialog path.");
        }
        if (!ponder::core::IsValidUtf8WithoutEmbeddedNull(pathText))
        {
            return MakeDialogFailure(context.kind, "SDL returned a dialog path that was not valid UTF-8.");
        }
    ...
    if (selectedFilterIndex.has_value() && *selectedFilterIndex >= context.filters.size())
    {
        return MakeDialogFailure(context.kind, "SDL returned an out-of-range selected dialog filter index.");
    }
...
void SDLCALL OnSdlDialogCompleted(void* userdata, const char* const* fileList, int selectedFilter) noexcept
{
    ...
    try
    {
        ...
        {
            const char* const rawError = SDL_GetError();
            const std::string error = rawError != nullptr ? std::string{rawError} : std::string{};
            context->Complete(timestamp, MakeDialogFailure(context->kind, error));
            return;
        }

        if (selectedFilter < -1)
        {
            context->Complete(timestamp, MakeDialogFailure(context->kind, "SDL returned an invalid negative selected dialog filter index."));
            return;
        }

        if (fileList[0] == nullptr)
        {
            context->Complete(timestamp, DialogCancellation{});
            return;
        }

        context->Complete(timestamp, MakeDialogSelection(*context, fileList, selectedFilter));
    }
    catch (const ponder::core::Exception& exception)
    {
        LOG_ERROR_CATEGORY(kLogCategory, "Dialog callback for request {} failed: {}", context->requestId, exception.GetMessage());
        context->MarkCallbackFailure(timestamp);
    }
    catch (const std::exception& exception)
    {
        LOG_ERROR_CATEGORY(kLogCategory, "Dialog callback for request {} failed: {}", context->requestId, exception.what());
        context->MarkCallbackFailure(timestamp);
    }
    catch (...)
    {
        LOG_ERROR_CATEGORY(kLogCategory, "Dialog callback for request {} failed with an unknown exception", context->requestId);
        context->MarkCallbackFailure(timestamp);
    }
}

void SubmitSdlDialog(std::unique_ptr<SdlDialogContext> context, std::optional<BackendWindowHandle> parentWindow) noexcept
{
    SDL_Window* const parent = parentWindow.has_value() ? ToSdlWindow(*parentWindow) : nullptr;
    ...
    SdlDialogContext* const callbackContext = context.release();

    switch (callbackContext->kind)
    {
    case dialogs::DialogKind::OpenFile:
        SDL_ShowOpenFileDialog(OnSdlDialogCompleted, callbackContext, parent, filters, filterCount, defaultLocation,
                               callbackContext->allowMultipleSelection);
        break;
    case dialogs::DialogKind::SaveFile:
        SDL_ShowSaveFileDialog(OnSdlDialogCompleted, callbackContext, parent, filters, filterCount, defaultLocation);
        break;
    case dialogs::DialogKind::OpenFolder:
        SDL_ShowOpenFolderDialog(OnSdlDialogCompleted, callbackContext, parent, defaultLocation, callbackContext->allowMultipleSelection);
        break;
    default:
        PONDER_UNREACHABLE("Cannot submit a dialog with unrecognized kind {}", callbackContext->kind);
    }
}
} // namespace

dialogs::DialogRequestId SdlRuntime::DialogShow(dialogs::DialogKind kind, std::optional<WindowId> parentWindowId,
                                                const std::optional<std::filesystem::path>& defaultLocation,
                                                std::span<const dialogs::DialogFileFilter> filters, bool allowMultipleSelection)
{
    PONDER_ASSERT(m_ownerThread.IsOwnerThread(), "Dialog requests must be submitted on the Runtime owner thread.");
    PONDER_ASSERT(m_initialized, "Cannot submit a dialog before Runtime initialization.");

    if (m_dialogCallbackHandoff == nullptr)
    {
        throw PLATFORM_EXCEPTION(PlatformErrorCode::InvalidArgument, "Cannot submit a dialog after dialog services shutdown.");
    }
    ...
    auto request = std::make_unique<DialogRequestState>();
    ...
    request->completion = DialogCompletionRecord{
        .timestamp = {},
    ...
    auto context = std::make_unique<SdlDialogContext>(m_dialogCallbackHandoff, id, request->info.kind, request->info.allowMultipleSelection,
                                                      std::move(prepared.defaultLocation), std::move(prepared.filters));

    {
        std::scoped_lock lock{m_dialogMutex};
        const auto [iterator, inserted] = m_dialogRequests.emplace(id, std::move(request));
    ...
    SubmitSdlDialog(std::move(context), parentBackendWindow);
    return id;
}

ponder::core::Result<dialogs::DialogRequestId> SdlRuntime::DialogShowOpenFile(const dialogs::OpenFileDialogDesc& desc) noexcept
{
    try
    {
        return DialogShow(dialogs::DialogKind::OpenFile, desc.parentWindowId, desc.defaultLocation, desc.filters, desc.allowMultipleSelection);
    }
    catch (...)
    {
        return ponder::core::Result<dialogs::DialogRequestId>::FromError(MakeDialogApiError("DialogShowOpenFile", std::current_exception()));
    }
}

ponder::core::Result<dialogs::DialogRequestId> SdlRuntime::DialogShowSaveFile(const dialogs::SaveFileDialogDesc& desc) noexcept
{
    try
    {
        return DialogShow(dialogs::DialogKind::SaveFile, desc.parentWindowId, desc.defaultLocation, desc.filters, false);
    }
    catch (...)
    {
        return ponder::core::Result<dialogs::DialogRequestId>::FromError(MakeDialogApiError("DialogShowSaveFile", std::current_exception()));
    }
}

ponder::core::Result<dialogs::DialogRequestId> SdlRuntime::DialogShowOpenFolder(const dialogs::OpenFolderDialogDesc& desc) noexcept
{
    try
    {
        return DialogShow(dialogs::DialogKind::OpenFolder, desc.parentWindowId, desc.defaultLocation, {}, desc.allowMultipleSelection);
    }
    catch (...)
    {
        return ponder::core::Result<dialogs::DialogRequestId>::FromError(MakeDialogApiError("DialogShowOpenFolder", std::current_exception()));
    }
}

std::size_t SdlRuntime::DialogGetPendingCount() const noexcept
{
...
bool SdlRuntime::DialogHasPending() const noexcept
{
    PONDER_ASSERT(m_ownerThread.IsOwnerThread(), "Dialog pending-state queries must run on the Runtime owner thread.");
    PONDER_ASSERT(m_initialized, "Cannot query pending dialogs before Runtime initialization.");
...
std::vector<DialogRequestInfo> SdlRuntime::DialogGetPending() const noexcept
{
    PONDER_ASSERT(m_ownerThread.IsOwnerThread(), "Dialog pending-state queries must run on the Runtime owner thread.");
    PONDER_ASSERT(m_initialized, "Cannot query pending dialogs before Runtime initialization.");
...
void SdlRuntime::DialogEnqueueCompletion(dialogs::DialogRequestId id, ponder::core::Timestamp timestamp, DialogOutcome outcome)
{
    {
        std::scoped_lock lock{m_dialogMutex};
        ...
        completedRequest.completion = DialogCompletionRecord{.timestamp = timestamp, .outcome = std::move(outcome)};
...
void SdlRuntime::DialogMarkCallbackFailure(dialogs::DialogRequestId id, ponder::core::Timestamp timestamp) noexcept
{
    try
    {
        {
            ...
            DialogRequestState& completedRequest = *request->second;
            PONDER_VERIFY(completedRequest.completionQueueNode.size() == 1, "Dialog request {} does not own exactly one completion node",
                          id.GetValue());
            completedRequest.completion.timestamp = timestamp;
            m_completedDialogRequests.splice(m_completedDialogRequests.end(), completedRequest.completionQueueNode);
            completedRequest.completionEnqueued = true;
        }
        DialogWakeNoThrow();
    ...
    {
        LOG_ERROR_CATEGORY(kLogCategory, "Could not enqueue the fallback failure for dialog request {}: {}", id, exception.what());
    }
    catch (...)
    {
        LOG_ERROR_CATEGORY(kLogCategory, "Could not enqueue the fallback failure for dialog request {} due to an unknown exception", id);
    }
}

std::optional<DialogCompletedEvent> SdlRuntime::DialogPollCompletion() noexcept
{
    ...
    std::unique_ptr<DialogRequestState> request;
    {
        std::scoped_lock lock{m_dialogMutex};
        if (m_completedDialogRequests.empty())
        {
            return std::optional<DialogCompletedEvent>{};
        }

        const dialogs::DialogRequestId completedRequestId = m_completedDialogRequests.front();
        ...
        request = std::move(completionIterator->second);
        m_completedDialogRequests.pop_front();
        m_dialogRequests.erase(completionIterator);
    }

    std::optional<DialogParentLease> parentLease = std::move(request->parentLease);
    DialogCompletedEvent event{.timestamp = request->completion.timestamp,
                               .request = std::move(request->info),
                               .outcome = std::move(request->completion.outcome)};
    parentLease.reset();
    return std::optional<DialogCompletedEvent>{std::move(event)};
}

std::size_t SdlRuntime::DialogGetOutstandingRequestCount() const noexcept
{
...
ponder::core::VoidResult SdlRuntime::DialogShutdown() noexcept
{
    PONDER_ASSERT(m_ownerThread.IsOwnerThread(), "Dialog shutdown must run on the Runtime owner thread.");
    PONDER_ASSERT(m_initialized, "Cannot shut down dialog services before Runtime initialization.");
    ...
    {
        return ponder::core::VoidResult::FromError(MakeDialogApiError("DialogShutdown", std::current_exception()));
    }
}

void SdlRuntime::DialogShutdownForRuntimeDestruction() noexcept
{
    const auto deactivateHandoff = [this]() noexcept
    {
        if (m_dialogCallbackHandoff == nullptr)
        {
            return;
        ...
        {
            LOG_ERROR_CATEGORY(kLogCategory, "Dialog callback handoff cleanup failed during runtime destruction: {}", exception.GetMessage());
        }
        catch (const std::exception& exception)
        {
            LOG_ERROR_CATEGORY(kLogCategory, "Dialog callback handoff cleanup failed during runtime destruction: {}", exception.what());
        }
        catch (...)
        {
            LOG_ERROR_CATEGORY(kLogCategory, "Dialog callback handoff cleanup failed during runtime destruction with an unknown failure");
        }
        m_dialogCallbackHandoff.reset();
    };
    ...
    ponder::core::VoidResult shutdown = DialogShutdown();
    if (!shutdown)
    {
        LOG_ERROR_CATEGORY(kLogCategory, "Dialog cleanup failed during runtime destruction: {}", shutdown.GetError());
    }
...
void SdlRuntime::Initialize(std::string_view applicationName, std::optional<std::string_view> applicationVersion,
                            std::optional<std::string_view> applicationIdentifier)
{
    ...
    setMetadata(ApplicationMetadataProperty::Name, ownedApplicationName.c_str());
    setMetadata(ApplicationMetadataProperty::Version, ownedApplicationVersion.has_value() ? ownedApplicationVersion->c_str() : nullptr);
    setMetadata(ApplicationMetadataProperty::Identifier, ownedApplicationIdentifier.has_value() ? ownedApplicationIdentifier->c_str() : nullptr);
    ...
    const Uint32 wakeEventType = SDL_RegisterEvents(2);
    if (wakeEventType == 0)
    {
        throw PLATFORM_EXCEPTION(PlatformErrorCode::BackendFailure, "{}", CaptureSdlFailureMessage("SDL_RegisterEvents", "runtime wake event"));
    }
    m_wakeEventType = static_cast<std::uint32_t>(wakeEventType);
    m_dialogWakeEventType = static_cast<std::uint32_t>(wakeEventType + 1U);
...
SdlRuntime::~SdlRuntime() noexcept
{
    ...
    std::size_t outstandingDialogCount{};
    if (m_initialized)
    {
        outstandingDialogCount = DialogGetOutstandingRequestCount();
    }
...
ponder::core::Timestamp SdlRuntime::TimeNow() const
{
    ...
    return ponder::core::Timestamp::Now();
}
...
{
    if (m_waitedBackendEvent.has_value())
    {
        event = std::move(*m_waitedBackendEvent);
        m_waitedBackendEvent.reset();
        return true;
    }
...
bool SdlRuntime::WaitBackendEvent(BackendEvent& event, std::int32_t timeoutMilliseconds) noexcept
{
    static_assert(std::is_trivially_copyable_v<SDL_Event>);
    static_assert(sizeof(SDL_Event) <= BackendEvent::kStorageSize);

    SDL_Event sdlEvent{};
    if (!SDL_WaitEventTimeout(&sdlEvent, static_cast<Sint32>(timeoutMilliseconds)))
    {
        return false;
    }
...
bool SdlRuntime::IsExternalWakeBackendEvent(const BackendEvent& event) const noexcept
{
    SDL_Event sdlEvent{};
    std::memcpy(&sdlEvent, event.m_storage.data(), sizeof(sdlEvent));
    return m_wakeEventType != 0 && sdlEvent.type == m_wakeEventType;
}

bool SdlRuntime::IsDialogWakeBackendEvent(const BackendEvent& event) const noexcept
{
    SDL_Event sdlEvent{};
    std::memcpy(&sdlEvent, event.m_storage.data(), sizeof(sdlEvent));
    return m_dialogWakeEventType != 0 && sdlEvent.type == m_dialogWakeEventType;
}
...
std::optional<PlatformEvent> SdlRuntime::TranslateBackendEvent(const BackendEvent& event, const EventTranslationContext& context) const
{
    SDL_Event sdlEvent{};
    std::memcpy(&sdlEvent, event.m_storage.data(), sizeof(sdlEvent));
    return TranslateSdlEvent(sdlEvent, ponder::core::Timestamp::Now(), context);
}
...
std::optional<PlatformEvent> SdlRuntime::EventPoll()
{
    ...
    std::optional<DialogCompletedEvent> dialogCompletion = DialogPollCompletion();
    if (dialogCompletion.has_value())
    {
    ...
    while (true)
    {
        ...
        if (IsExternalWakeBackendEvent(event))
        {
            m_wakeObserved = true;
            continue;
        }
        if (IsDialogWakeBackendEvent(event))
        {
            dialogCompletion = DialogPollCompletion();
            if (dialogCompletion.has_value())
            {
                return PlatformEvent{std::move(*dialogCompletion)};
            }
...
std::optional<PlatformEvent> SdlRuntime::EventWait(ponder::core::Duration timeout)
{
    VerifyOwnerThread("event waiting");
    const std::int32_t timeoutMilliseconds = GetEventWaitTimeoutMilliseconds(timeout);
    const ponder::core::Timestamp waitStartedAt = ponder::core::Timestamp::Now();

    while (true)
    {
        if (std::optional<PlatformEvent> event = EventPoll(); event.has_value())
        {
            m_wakeObserved = false;
            return event;
        }
        if (std::exchange(m_wakeObserved, false))
        {
            return std::nullopt;
        }
        ...
        const ponder::core::Duration elapsed = ponder::core::Timestamp::Now() - waitStartedAt;
        if (elapsed >= timeout)
        {
            return std::nullopt;
        }

        const ponder::core::Duration remaining{timeout.GetNanoseconds() - elapsed.GetNanoseconds()};
        const std::int32_t backendTimeout = GetEventWaitTimeoutMilliseconds(remaining);

        BackendEvent waitedEvent;
        if (!WaitBackendEvent(waitedEvent, backendTimeout))
        {
            return std::nullopt;
        }
        if (IsExternalWakeBackendEvent(waitedEvent))
        {
            return std::nullopt;
        }
        if (IsDialogWakeBackendEvent(waitedEvent))
        {
            std::optional<DialogCompletedEvent> dialogCompletion = DialogPollCompletion();
            if (dialogCompletion.has_value())
            {
        ...
        PONDER_VERIFY(!m_waitedBackendEvent.has_value(), "Cannot buffer more than one waited backend event");
        m_waitedBackendEvent.emplace(std::move(waitedEvent));
    }
}

void SdlRuntime::EventWake()
{
    ...
    {
        throw PLATFORM_EXCEPTION(PlatformErrorCode::BackendFailure, "{}", CaptureSdlFailureMessage("SDL_PushEvent", "runtime wake event"));
    }
}

void SdlRuntime::DialogWakeNoThrow() noexcept
{
    if (m_dialogWakeEventType == 0)
    {
        LOG_ERROR_CATEGORY(kLogCategory, "Cannot wake the event loop for a dialog completion before Runtime initialization");
        return;
    }

    SDL_Event event{};
    event.type = m_dialogWakeEventType;
    if (!SDL_PushEvent(&event))
    {
        const char* const error = SDL_GetError();
        LOG_ERROR_CATEGORY(kLogCategory, "Failed to wake the event loop for a dialog completion: {}", error != nullptr ? error : "unknown SDL error");
    }
}
