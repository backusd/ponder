...
[[nodiscard]] core::Error MakeNativeHandleError(
    core::ErrorCode code, std::string_view message, std::string_view context)
{
    return core::Error{
        code, std::string{message} + " Context: " + std::string{context} + "."};
}
} // namespace
...
core::Result<NativeWindowHandle> WindowImpl::GetNativeHandle() const
{
    ...
    const BackendNativeWindowHandleResult result = m_backend.getNativeHandle(
        m_backend.context, m_nativeWindow, &m_cocoaMetalView, &handle);
    ...
    case BackendNativeWindowHandleStatus::Unsupported:
        return core::Result<NativeWindowHandle>::FromError(MakeNativeHandleError(
            ...
            result.message != nullptr
                ? std::string_view{result.message}
    ...
    case BackendNativeWindowHandleStatus::Failed:
        if (result.captureSdlError)
        {
            return core::Result<NativeWindowHandle>::FromError(CaptureSdlFailure(
                kBackendFailureCode,
                result.operation != nullptr ? result.operation
                                            : "SDL native window handle query",
                GetErrorContext()));
        }
        return core::Result<NativeWindowHandle>::FromError(MakeNativeHandleError(
            kBackendFailureCode,
