...
{
public:
    [[nodiscard]] static constexpr core::Result<AxisAlignedBox> Create(Vector3 minimum, Vector3 maximum)
    {
        if (!detail::IsFiniteAxisAlignedBoxVector(minimum) || !detail::IsFiniteAxisAlignedBoxVector(maximum)) [[unlikely]]
        {
            return core::Result<AxisAlignedBox>::FromError(
                core::Error{ToErrorCode(MathErrorCode::NonFiniteInput), "AxisAlignedBox construction requires finite bounds."});
        }

        if (maximum.x < minimum.x || maximum.y < minimum.y || maximum.z < minimum.z) [[unlikely]]
        {
            return core::Result<AxisAlignedBox>::FromError(
                core::Error{ToErrorCode(MathErrorCode::InvalidArgument), "AxisAlignedBox construction requires ordered minimum and maximum bounds."});
        }
    ...
    [[nodiscard]] friend constexpr bool operator==(const AxisAlignedBox& lhs, const AxisAlignedBox& rhs) noexcept = default;
