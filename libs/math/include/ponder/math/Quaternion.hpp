...
public:
    ...
    [[nodiscard]] static inline core::Result<Quaternion> FromComponents(float x, float y, float z, float w)
    {
    ...
    [[nodiscard]] static inline core::Result<Quaternion> FromAxisAngle(Vector3 axis, Radians angle)
    {
        if (!core::IsFinite(angle.GetValue())) [[unlikely]]
        {
            return core::Result<Quaternion>::FromError(
                core::Error{ToErrorCode(MathErrorCode::NonFiniteInput), "Quaternion axis-angle construction requires a finite angle."});
        }
        ...
        return FromComponents(normalizedAxis->x * sine, normalizedAxis->y * sine, normalizedAxis->z * sine, cosine);
    }
    ...
    [[nodiscard]] friend constexpr bool operator==(const Quaternion& lhs, const Quaternion& rhs) noexcept = default;
...
private:
    ...
    [[nodiscard]] static inline core::Result<Quaternion> NormalizeFiniteComponents(float x, float y, float z, float w)
    {
        if (!core::IsFinite(x) || !core::IsFinite(y) || !core::IsFinite(z) || !core::IsFinite(w)) [[unlikely]]
        {
            return core::Result<Quaternion>::FromError(
                core::Error{ToErrorCode(MathErrorCode::NonFiniteInput), "Quaternion construction requires finite components."});
        }
        ...
        {
            return core::Result<Quaternion>::FromError(
                core::Error{ToErrorCode(MathErrorCode::DegenerateInput), "Quaternion construction requires non-zero length."});
        }

        const Quaternion result = NormalizeFiniteComponentsUnchecked(x, y, z, w);
        if (!core::IsFinite(result.GetX()) || !core::IsFinite(result.GetY()) || !core::IsFinite(result.GetZ()) || !core::IsFinite(result.GetW()))
            [[unlikely]]
        {
            return core::Result<Quaternion>::FromError(
                core::Error{ToErrorCode(MathErrorCode::DegenerateInput), "Quaternion construction result is numerically unnormalizable."});
        }

        return result;
    }

    [[nodiscard]] static inline Quaternion NormalizeFiniteComponentsUnchecked(float x, float y, float z, float w) noexcept
    {
        const float maxMagnitude = Max(Max(Abs(x), Abs(y)), Max(Abs(z), Abs(w)));
        ...
        const double length = std::sqrt(scaledX * scaledX + scaledY * scaledY + scaledZ * scaledZ + scaledW * scaledW);
        const double inverseLength = 1.0 / length;

        return Quaternion{static_cast<float>(scaledX * inverseLength), static_cast<float>(scaledY * inverseLength),
                          static_cast<float>(scaledZ * inverseLength), static_cast<float>(scaledW * inverseLength)};
    }
...
[[nodiscard]] constexpr bool IsNear(Quaternion lhs, Quaternion rhs, core::Tolerance tolerance) noexcept
{
    return core::IsNear(lhs.GetX(), rhs.GetX(), tolerance) && core::IsNear(lhs.GetY(), rhs.GetY(), tolerance) &&
           core::IsNear(lhs.GetZ(), rhs.GetZ(), tolerance) && core::IsNear(lhs.GetW(), rhs.GetW(), tolerance);
}

[[nodiscard]] constexpr bool IsSameRotation(Quaternion lhs, Quaternion rhs, core::Tolerance tolerance) noexcept
{
    return IsNear(lhs, rhs, tolerance) || (core::IsNear(lhs.GetX(), -rhs.GetX(), tolerance) && core::IsNear(lhs.GetY(), -rhs.GetY(), tolerance) &&
                                           core::IsNear(lhs.GetZ(), -rhs.GetZ(), tolerance) && core::IsNear(lhs.GetW(), -rhs.GetW(), tolerance));
}

[[nodiscard]] inline Quaternion operator*(Quaternion lhs, Quaternion rhs) noexcept
{
    return Quaternion::NormalizeFiniteComponentsUnchecked(lhs.m_w * rhs.m_x + lhs.m_x * rhs.m_w + lhs.m_y * rhs.m_z - lhs.m_z * rhs.m_y,
...
[[nodiscard]] constexpr core::Result<Quaternion> Slerp(Quaternion start, Quaternion end, float amount)
{
    if (!core::IsFinite(amount)) [[unlikely]]
    {
        return core::Result<Quaternion>::FromError(
            core::Error{ToErrorCode(MathErrorCode::NonFiniteInput), "Quaternion interpolation requires a finite amount."});
    }
    ...
    {
        return Quaternion::NormalizeFiniteComponents(start.m_x + amount * (endX - start.m_x), start.m_y + amount * (endY - start.m_y),
    ...
    return Quaternion::NormalizeFiniteComponents(
        static_cast<float>(startWeight * start.m_x + endWeight * endX), static_cast<float>(startWeight * start.m_y + endWeight * endY),
        static_cast<float>(startWeight * start.m_z + endWeight * endZ), static_cast<float>(startWeight * start.m_w + endWeight * endW));
}
...
[[nodiscard]] constexpr Matrix3x3 ToMatrix3x3(Quaternion rotation) noexcept
{
    ...
    return Matrix3x3{1.0F - 2.0F * (yy + zz), 2.0F * (xy - wz), 2.0F * (xz + wy), 2.0F * (xy + wz),       1.0F - 2.0F * (xx + zz),
...
namespace detail
{
[[nodiscard]] constexpr core::Result<void> ValidateRotationMatrix(Matrix3x3 matrix, core::Tolerance tolerance)
{
    if (!IsFinite(matrix)) [[unlikely]]
    {
        return core::Result<void>::FromError(
            core::Error{ToErrorCode(MathErrorCode::NonFiniteInput), "Quaternion conversion requires finite matrix components."});
    }
    ...
    {
        return core::Result<void>::FromError(
            core::Error{ToErrorCode(MathErrorCode::SingularMatrix), "Quaternion conversion requires a nonsingular rotation matrix."});
    }
    ...
    if (!core::IsNear(Dot(column0, column0), 1.0F, tolerance) || !core::IsNear(Dot(column1, column1), 1.0F, tolerance) ||
        !core::IsNear(Dot(column2, column2), 1.0F, tolerance) || !core::IsNear(Dot(column0, column1), 0.0F, tolerance) ||
        !core::IsNear(Dot(column0, column2), 0.0F, tolerance) || !core::IsNear(Dot(column1, column2), 0.0F, tolerance) ||
        !core::IsNear(determinant, 1.0F, tolerance)) [[unlikely]]
    {
        return core::Result<void>::FromError(
            core::Error{ToErrorCode(MathErrorCode::DegenerateInput), "Quaternion conversion requires an orthonormal proper rotation matrix."});
    }
...
[[nodiscard]] inline core::Result<Quaternion> ToQuaternion(Matrix3x3 matrix, core::Tolerance tolerance)
{
    ...
    if (trace > 0.0F)
    {
        ...
        return Quaternion::FromComponents(static_cast<float>((matrix.row2Column1 - matrix.row1Column2) / scale),
                                          static_cast<float>((matrix.row0Column2 - matrix.row2Column0) / scale),
                                          static_cast<float>((matrix.row1Column0 - matrix.row0Column1) / scale), static_cast<float>(0.25 * scale));
    }

    if (matrix.row0Column0 >= matrix.row1Column1 && matrix.row0Column0 >= matrix.row2Column2)
    {
        const double scale = std::sqrt(1.0 + static_cast<double>(matrix.row0Column0) - static_cast<double>(matrix.row1Column1) -
                                       static_cast<double>(matrix.row2Column2)) *
                             2.0;
        return Quaternion::FromComponents(static_cast<float>(0.25 * scale), static_cast<float>((matrix.row0Column1 + matrix.row1Column0) / scale),
                                          static_cast<float>((matrix.row0Column2 + matrix.row2Column0) / scale),
    ...
    {
        const double scale = std::sqrt(1.0 + static_cast<double>(matrix.row1Column1) - static_cast<double>(matrix.row0Column0) -
                                       static_cast<double>(matrix.row2Column2)) *
                             2.0;
        return Quaternion::FromComponents(static_cast<float>((matrix.row0Column1 + matrix.row1Column0) / scale), static_cast<float>(0.25 * scale),
                                          static_cast<float>((matrix.row1Column2 + matrix.row2Column1) / scale),
                                          static_cast<float>((matrix.row0Column2 - matrix.row2Column0) / scale));
    }

    const double scale =
        std::sqrt(1.0 + static_cast<double>(matrix.row2Column2) - static_cast<double>(matrix.row0Column0) - static_cast<double>(matrix.row1Column1)) *
        2.0;
    return Quaternion::FromComponents(static_cast<float>((matrix.row0Column2 + matrix.row2Column0) / scale),
                                      static_cast<float>((matrix.row1Column2 + matrix.row2Column1) / scale), static_cast<float>(0.25 * scale),
                                      static_cast<float>((matrix.row1Column0 - matrix.row0Column1) / scale));
}

[[nodiscard]] inline core::Result<Quaternion> ToQuaternion(Matrix4x4 matrix, core::Tolerance tolerance)
{
    return ToQuaternion(Matrix3x3{matrix.row0Column0, matrix.row0Column1, matrix.row0Column2, matrix.row1Column0, matrix.row1Column1,
                                  matrix.row1Column2, matrix.row2Column0, matrix.row2Column1, matrix.row2Column2},
                        tolerance);
}
} // namespace pond::math
