...
struct SrgbStraightAlphaColor final
{
    ...
    [[nodiscard]] friend constexpr bool operator==(const SrgbStraightAlphaColor& lhs, const SrgbStraightAlphaColor& rhs) noexcept = default;
};
...
struct LinearPremultipliedColor final
{
    ...
    [[nodiscard]] friend constexpr bool operator==(const LinearPremultipliedColor& lhs, const LinearPremultipliedColor& rhs) noexcept = default;
};
...
public:
    ...
    explicit constexpr PackedLinearPremultipliedRgba8(std::uint32_t value) noexcept :
        m_value{value}
    {
    }

    [[nodiscard]] static constexpr PackedLinearPremultipliedRgba8 FromChannels(std::uint8_t red, std::uint8_t green, std::uint8_t blue,
                                                                               std::uint8_t alpha) noexcept
    {
        return PackedLinearPremultipliedRgba8{static_cast<std::uint32_t>(red) | (static_cast<std::uint32_t>(green) << 8U) |
    ...
    [[nodiscard]] friend constexpr bool operator==(const PackedLinearPremultipliedRgba8& lhs,
...
{
    return IsUnitInterval(color.red) && IsUnitInterval(color.green) && IsUnitInterval(color.blue) && IsUnitInterval(color.alpha);
}

[[nodiscard]] constexpr bool IsValid(LinearPremultipliedColor color) noexcept
{
    return IsUnitInterval(color.alpha) && core::IsFinite(color.red) && core::IsFinite(color.green) && core::IsFinite(color.blue) &&
           color.red >= 0.0F && color.green >= 0.0F && color.blue >= 0.0F && color.red <= color.alpha && color.green <= color.alpha &&
...
[[nodiscard]] inline core::Result<SrgbStraightAlphaColor> MakeSrgbStraightAlphaColor(float red, float green, float blue, float alpha)
{
    const SrgbStraightAlphaColor color{red, green, blue, alpha};
    if (!IsValid(color))
    {
        return MakeUiFailure<SrgbStraightAlphaColor>(UiErrorCode::InvalidPaintValue,
...
[[nodiscard]] inline core::Result<LinearPremultipliedColor> MakeLinearPremultipliedColor(float red, float green, float blue, float alpha)
{
    const LinearPremultipliedColor color{red, green, blue, alpha};
    if (!IsValid(color))
    {
        return MakeUiFailure<LinearPremultipliedColor>(UiErrorCode::InvalidPaintValue,
...
[[nodiscard]] inline LinearPremultipliedColor ToLinearPremultiplied(SrgbStraightAlphaColor color) noexcept
{
    return LinearPremultipliedColor{ConvertSrgbChannelToLinear(color.red) * color.alpha, ConvertSrgbChannelToLinear(color.green) * color.alpha,
                                    ConvertSrgbChannelToLinear(color.blue) * color.alpha, color.alpha};
}

[[nodiscard]] inline LinearPremultipliedColor BlendSourceOver(LinearPremultipliedColor source, LinearPremultipliedColor destination) noexcept
{
    const float destinationWeight = 1.0F - source.alpha;
    return LinearPremultipliedColor{source.red + destination.red * destinationWeight, source.green + destination.green * destinationWeight,
                                    source.blue + destination.blue * destinationWeight, source.alpha + destination.alpha * destinationWeight};
}
...
[[nodiscard]] inline core::Result<std::uint8_t> QuantizeUnitFloatToUnorm8(float value)
{
    ...
    {
        return MakeUiFailure<std::uint8_t>(UiErrorCode::InvalidPaintValue, "UI UNORM8 quantization requires a finite value in [0, 1].");
    }
...
[[nodiscard]] inline core::Result<PackedLinearPremultipliedRgba8> PackLinearPremultipliedRgba8(LinearPremultipliedColor color)
{
    if (!IsValid(color))
    {
        return MakeUiFailure<PackedLinearPremultipliedRgba8>(UiErrorCode::InvalidPaintValue,
