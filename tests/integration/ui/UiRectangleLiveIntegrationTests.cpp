...
inline constexpr render::ClearColor kContrastingClearColor{.red = 0.025F, .green = 0.045F, .blue = 0.085F, .alpha = 1.0F};
...
{
    return core::Error{render::ToErrorCode(render::RenderErrorCode::BackendFailure), std::move(message)};
}
...
[[nodiscard]] std::array<experimental::RectanglePaint, 3U> MakeRectangleScene(const ui::UiTargetMetrics& metrics) noexcept
{
    const ui::LogicalSize extent = metrics.GetLogicalSize();
    return {experimental::RectanglePaint{.rectangle = {.origin = {.x = extent.width * 0.08F, .y = extent.height * 0.10F},
                                                       .size = {.width = extent.width * 0.56F, .height = extent.height * 0.66F}},
                                         .color = {.red = 0.16F, .green = 0.48F, .blue = 0.92F, .alpha = 1.0F}},
            experimental::RectanglePaint{.rectangle = {.origin = {.x = extent.width * 0.28F, .y = extent.height * 0.22F},
                                                       .size = {.width = extent.width * 0.48F, .height = extent.height * 0.58F}},
                                         .color = {.red = 1.0F, .green = 0.22F, .blue = 0.06F, .alpha = 0.62F}},
            experimental::RectanglePaint{.rectangle = {.origin = {.x = extent.width * 0.45F, .y = extent.height * 0.36F},
                                                       .size = {.width = extent.width * 0.45F, .height = extent.height * 0.52F}},
                                         .color = {.red = 0.02F, .green = 0.92F, .blue = 0.65F, .alpha = 0.58F}}};
}

[[nodiscard]] core::Result<PresentedRectangleFrame> PresentRectangleFrame(render::RenderTarget& target,
                                                                          experimental::RectangleRenderer& rectangleRenderer)
{
    ...
    for (std::size_t attempt = 0U; attempt < kMaximumFrameAttempts; ++attempt)
    {
        ...
        if (!frameResult)
        {
            return core::Result<PresentedRectangleFrame>::FromError(std::move(frameResult).GetError());
        }
        ...
        if (!metricsResult)
        {
            return core::Result<PresentedRectangleFrame>::FromError(std::move(metricsResult).GetError());
        }
        ...
        if (!recordResult)
        {
            return core::Result<PresentedRectangleFrame>::FromError(std::move(recordResult).GetError());
        }

        auto presentationResult = frame.FinishAndPresent();
        if (!presentationResult)
        {
            return core::Result<PresentedRectangleFrame>::FromError(std::move(presentationResult).GetError());
        }
        ...
        if (presentation.presented)
        {
            ...
            {
                return MakeLiveTestFailure<PresentedRectangleFrame>("A presented rectangle frame did not record its production Draw2D stage.");
            }
            return PresentedRectangleFrame{.metrics = metrics, .presentation = presentation};
        }

        if (presentation.status != render::FrameStatus::TimedOut && presentation.status != render::FrameStatus::RecreationPending &&
            presentation.status != render::FrameStatus::SkippedSuspended)
        {
            return MakeLiveTestFailure<PresentedRectangleFrame>("A live rectangle frame completed with an unexpected non-presented status.");
        }
    }

    return MakeLiveTestFailure<PresentedRectangleFrame>("No live rectangle frame was presented within the bounded attempt count.");
}

[[nodiscard]] core::VoidResult ExerciseSuspendedRectangleFrame(render::RenderTarget& target, experimental::RectangleRenderer& rectangleRenderer)
{
    auto frameResult = target.AcquireFrame();
    if (!frameResult)
    {
    ...
    {
        return core::VoidResult::FromError(MakeLiveTestError("A minimized rectangle frame did not report a suspended skip."));
    }
    ...
    if (presentationResult.GetValue().presented || presentationResult.GetValue().status != render::FrameStatus::SkippedSuspended)
    {
        return core::VoidResult::FromError(MakeLiveTestError("A minimized rectangle frame unexpectedly presented or returned the wrong status."));
    }

    return core::VoidResult::Success();
}

void ExpectMetricsMatchSnapshot(const ui::UiTargetMetrics& metrics, const render::RenderTargetSnapshot& snapshot)
{
    ...
    EXPECT_EQ(metrics.GetMetricsRevision().GetValue(), snapshot.GetPresentationEnvironmentRevision().GetValue());
...
void ExpectValidationConfiguration(const render::RenderBootstrap& bootstrap)
{
    ...
    if (requested == render::RenderValidationMode::Default)
    {
        ...
        EXPECT_EQ(diagnostics.validationEnabled, diagnostics.enabledValidationMode == render::RenderValidationMode::Standard);
    }
...
TEST(UiRectangleLiveIntegrationTests, ExercisesProductionPathLifecycleAndSteadyState)
{
    ...
    if (!runtimeResult)
    {
        if (auto reason = render::test::MakeOptionalLiveSkipReason(runtimeResult.GetError(), render::test::LivePrerequisiteOperation::PlatformRuntime,
    ...
    const platform::WindowDesc firstWindowDesc{.title = "Ponder UI Live Rectangle A",
                                               .logicalSize = {.width = 480U, .height = 320U},
                                               .visible = true,
                                               .resizable = true,
                                               .highPixelDensity = true,
                                               .graphicsCompatibility = render::GetRequiredWindowGraphicsCompatibility()};
    const platform::WindowDesc secondWindowDesc{.title = "Ponder UI Live Rectangle B",
    ...
    if (!firstWindowResult)
    {
        if (auto reason = render::test::MakeOptionalLiveSkipReason(
                firstWindowResult.GetError(), render::test::LivePrerequisiteOperation::PlatformWindow, "First UI live Vulkan window creation"))
        {
            GTEST_SKIP() << *reason;
        }
    ...
    {
        if (auto reason = render::test::MakeOptionalLiveSkipReason(
                secondWindowResult.GetError(), render::test::LivePrerequisiteOperation::PlatformWindow, "Second UI live Vulkan window creation"))
        {
            GTEST_SKIP() << *reason;
        }
    ...
    auto bootstrapResult = render::RenderBootstrap::Create(render::test::MakeLiveRenderBootstrapDesc());
    if (!bootstrapResult)
    {
        if (auto reason = render::test::MakeOptionalLiveSkipReason(bootstrapResult.GetError(), render::test::LivePrerequisiteOperation::Bootstrap,
    ...
    auto firstSurfaceResult = bootstrap.PrepareSurface(
        *firstWindow, render::SurfacePreparationDesc{.targetSnapshot = firstSnapshot, .reason = render::SurfacePreparationReason::Initial});
    if (!firstSurfaceResult)
    {
        if (auto reason = render::test::MakeOptionalLiveSkipReason(firstSurfaceResult.GetError(), render::test::LivePrerequisiteOperation::Surface,
    ...
    auto secondSurfaceResult = bootstrap.PrepareSurface(
        secondWindow, render::SurfacePreparationDesc{.targetSnapshot = secondSnapshot, .reason = render::SurfacePreparationReason::Initial});
    if (!secondSurfaceResult)
    {
        if (auto reason = render::test::MakeOptionalLiveSkipReason(secondSurfaceResult.GetError(), render::test::LivePrerequisiteOperation::Surface,
    ...
    auto selectionResult = bootstrap.SelectAdapter(firstSurface, render::RenderAdapterSelectionDesc{});
    if (!selectionResult)
    {
        if (auto reason = render::test::MakeOptionalLiveSkipReason(selectionResult.GetError(), render::test::LivePrerequisiteOperation::Adapter,
    ...
    auto deviceResult = bootstrap.CreateDevice(firstSurface, selectionResult.GetValue(), render::RenderDeviceDesc{});
    if (!deviceResult)
    {
        if (auto reason = render::test::MakeOptionalLiveSkipReason(deviceResult.GetError(), render::test::LivePrerequisiteOperation::Device,
    ...
    auto firstTargetResult = device.CreateRenderTarget(
        std::move(firstSurface), render::RenderTargetDesc{.targetSnapshot = firstSnapshot, .clearColor = kContrastingClearColor});
    if (!firstTargetResult)
    {
        if (auto reason = render::test::MakeOptionalLiveSkipReason(firstTargetResult.GetError(), render::test::LivePrerequisiteOperation::Target,
    ...
    auto secondTargetResult = device.CreateRenderTarget(
        std::move(secondSurface), render::RenderTargetDesc{.targetSnapshot = secondSnapshot, .clearColor = kContrastingClearColor});
    if (!secondTargetResult)
    {
        if (auto reason = render::test::MakeOptionalLiveSkipReason(secondTargetResult.GetError(), render::test::LivePrerequisiteOperation::Target,
    ...
    experimental::RectangleRenderer rectangleRenderer = std::move(rectangleRendererResult).GetValue();
    EXPECT_EQ(render::detail::RenderLiveTestAccess::GetDraw2DDeviceStats(device).activeLayerCount, 1U);
    ...
    const auto warmedDeviceStats = render::detail::RenderLiveTestAccess::GetDraw2DDeviceStats(device);
    const auto warmedFirstTargetStats = render::detail::RenderLiveTestAccess::GetDraw2DTargetStats(firstTarget);
    const auto warmedSecondTargetStats = render::detail::RenderLiveTestAccess::GetDraw2DTargetStats(secondTarget);
    ...
    const auto steadyDeviceStats = render::detail::RenderLiveTestAccess::GetDraw2DDeviceStats(device);
    const auto steadyFirstTargetStats = render::detail::RenderLiveTestAccess::GetDraw2DTargetStats(firstTarget);
    const auto steadySecondTargetStats = render::detail::RenderLiveTestAccess::GetDraw2DTargetStats(secondTarget);
    EXPECT_EQ(steadyDeviceStats.pipelineCreationCount, warmedDeviceStats.pipelineCreationCount);
    EXPECT_EQ(steadyDeviceStats.pipelineReplacementCount, warmedDeviceStats.pipelineReplacementCount);
    EXPECT_GE(steadyDeviceStats.pipelineReuseCount, warmedDeviceStats.pipelineReuseCount + (2U * kSteadyFramesPerTarget));
    EXPECT_EQ(steadyFirstTargetStats.currentCapacityBytes, warmedFirstTargetStats.currentCapacityBytes);
    ...
    EXPECT_EQ(steadySecondTargetStats.currentCapacityBytes, warmedSecondTargetStats.currentCapacityBytes);
    ...
    constexpr std::array<platform::LogicalSize, 4U> kResizeStorm{
        platform::LogicalSize{.width = 520U, .height = 340U}, platform::LogicalSize{.width = 560U, .height = 360U},
        platform::LogicalSize{.width = 540U, .height = 350U}, platform::LogicalSize{.width = 600U, .height = 400U}};
    ...
    ASSERT_TRUE(render::test::PumpPlatformEventsUntil(
        ...
        [&firstEvents, &firstWindow]()
        {
            ...
            return firstEvents.GetResizeEventCount() > 0U && logicalSize &&
                   logicalSize.GetValue() == platform::LogicalSize{.width = 600U, .height = 400U} && pixelSize && pixelSize.GetValue().width > 0U &&
                   pixelSize.GetValue().height > 0U;
        },
    ...
    ASSERT_TRUE(render::test::PumpPlatformEventsUntil(
        ...
        [&firstEvents, &firstWindow, stateChangesBeforeMinimize]()
        {
            ...
            return firstEvents.stateChanges > stateChangesBeforeMinimize && state && state.GetValue() == platform::WindowState::Minimized;
        },
        kPlatformEventTimeout));
    ...
    const core::VoidResult suspendedFrame = ExerciseSuspendedRectangleFrame(firstTarget, rectangleRenderer);
    ...
    ASSERT_TRUE(render::test::PumpPlatformEventsUntil(
        ...
        [&firstEvents, &firstWindow, stateChangesBeforeRestore]()
        {
            const auto state = firstWindow->GetState();
            const auto pixelSize = firstWindow->GetPixelSize();
            return firstEvents.stateChanges > stateChangesBeforeRestore && state && state.GetValue() != platform::WindowState::Minimized &&
    ...
    EXPECT_EQ(render::detail::RenderLiveTestAccess::GetDraw2DDeviceStats(device).activeLayerCount, 0U);
    ...
    EXPECT_TRUE(validationReport.IsClean()) << render::test::FormatValidationReport(validationReport);
    EXPECT_EQ(validationReport.droppedMessageCount, 0U) << render::test::FormatValidationReport(validationReport);
