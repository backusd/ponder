...
[[nodiscard]] pond::render::RenderTargetSnapshot MakeSnapshot(
    const ponder::platform::Window& window, std::uint64_t revision,
    pond::render::PresentationEnvironmentRevision presentationEnvironmentRevision = pond::render::PresentationEnvironmentRevision{1U})
{
    ...
    return pond::render::RenderTargetSnapshot{
        window.GetId(), pixelSize.GetValue(), logicalSize.GetValue(), visible.GetValue(), windowState.GetValue(), presentationEnvironmentRevision,
        revision};
}
...
#if defined(_WIN32)
[[nodiscard]] bool RequestNativeClose(ponder::platform::Window& window)
{
    ...
    const auto* win32 = std::get_if<ponder::platform::NativeWin32Window>(&nativeHandle.GetValue());
    return win32 != nullptr && win32->window != nullptr && PostMessageW(static_cast<HWND>(win32->window), WM_CLOSE, 0U, 0) != FALSE;
}
#endif

[[nodiscard]] pond::render::RenderTargetSnapshot MakeWindowStateSnapshot(
    const ponder::platform::Window& window, ponder::platform::PixelSize pixelSize, bool visible, ponder::platform::WindowState windowState,
    std::uint64_t revision,
    pond::render::PresentationEnvironmentRevision presentationEnvironmentRevision = pond::render::PresentationEnvironmentRevision{1U})
{
    return pond::render::RenderTargetSnapshot{window.GetId(), pixelSize,   ponder::platform::LogicalSize{pixelSize.width, pixelSize.height},
                                              visible,        windowState, presentationEnvironmentRevision,
                                              revision};
}

[[nodiscard]] constexpr pond::render::ClearColor MakeClearColor(float red, float green, float blue) noexcept
{
    return pond::render::ClearColor{.red = red, .green = green, .blue = blue, .alpha = 1.0F};
}

[[nodiscard]] pond::core::Result<pond::render::PreparedSurface> PrepareTransferredSurface(pond::render::RenderBootstrap& bootstrap,
                                                                                          ponder::platform::Window& window,
                                                                                          ...
                                                                                          pond::render::RenderTargetSnapshot transferSnapshot)
{
    auto surfaceResult = bootstrap.PrepareSurface(
        window, pond::render::SurfacePreparationDesc{.targetSnapshot = initialSnapshot, .reason = pond::render::SurfacePreparationReason::Initial});
    if (!surfaceResult)
    {
        return pond::core::Result<pond::render::PreparedSurface>::FromError(std::move(surfaceResult).GetError());
    }

    pond::render::PreparedSurface surface = std::move(surfaceResult).GetValue();
    if (pond::core::VoidResult transfer = surface.TransferToCurrentThread(transferSnapshot); !transfer)
    {
        return pond::core::Result<pond::render::PreparedSurface>::FromError(std::move(transfer).GetError());
    }
                          ...
                          pond::render::RenderFrameResult* observedResult = nullptr)
{
    ...
    for (std::size_t attempt = 0U; attempt < kMaximumFrameAttempts; ++attempt)
    {
        ...
        if (result.presented)
        {
            EXPECT_TRUE(result.status == pond::render::FrameStatus::Presented || result.status == pond::render::FrameStatus::Suboptimal ||
        ...
        ASSERT_TRUE(result.status == pond::render::FrameStatus::TimedOut || result.status == pond::render::FrameStatus::RecreationPending ||
...
{
    auto runtimeResult = ponder::platform::PlatformRuntime::Create(ponder::platform::PlatformRuntimeDesc{});
    if (!runtimeResult)
    {
        if (auto reason = pond::render::test::MakeOptionalLiveSkipReason(
                runtimeResult.GetError(), pond::render::test::LivePrerequisiteOperation::PlatformRuntime, "Platform runtime creation"))
        {
            GTEST_SKIP() << *reason;
        }
        FAIL() << runtimeResult.GetError().GetMessage();
    }
    ponder::platform::PlatformRuntime runtime = std::move(runtimeResult).GetValue();

    ponder::platform::WindowDesc windowDesc;
    ...
    {
        if (auto reason = pond::render::test::MakeOptionalLiveSkipReason(
                windowResult.GetError(), pond::render::test::LivePrerequisiteOperation::PlatformWindow, "Vulkan-compatible window creation"))
        {
            GTEST_SKIP() << *reason;
        }
        FAIL() << windowResult.GetError().GetMessage();
    }
    ponder::platform::Window window = std::move(windowResult).GetValue();
    ...
    auto bootstrapResult = pond::render::RenderBootstrap::Create(pond::render::test::MakeLiveRenderBootstrapDesc());
    if (!bootstrapResult)
    {
        if (auto reason = pond::render::test::MakeOptionalLiveSkipReason(
                bootstrapResult.GetError(), pond::render::test::LivePrerequisiteOperation::Bootstrap, "Render bootstrap creation"))
        {
            GTEST_SKIP() << *reason;
        }
    ...
    {
        if (auto reason = pond::render::test::MakeOptionalLiveSkipReason(
                surfaceResult.GetError(), pond::render::test::LivePrerequisiteOperation::Surface, "Vulkan surface preparation"))
        {
            GTEST_SKIP() << *reason;
        }
    ...
    {
        if (auto reason = pond::render::test::MakeOptionalLiveSkipReason(
                selectionResult.GetError(), pond::render::test::LivePrerequisiteOperation::Adapter, "Vulkan adapter selection"))
        {
            GTEST_SKIP() << *reason;
        }
    ...
    auto deviceResult = bootstrap.CreateDevice(surface, selectionResult.GetValue(), pond::render::RenderDeviceDesc{});
    if (!deviceResult)
    {
        if (auto reason = pond::render::test::MakeOptionalLiveSkipReason(
                deviceResult.GetError(), pond::render::test::LivePrerequisiteOperation::Device, "Vulkan logical-device creation"))
        {
            GTEST_SKIP() << *reason;
        }
    ...
    auto targetResult = device.CreateRenderTarget(std::move(surface), pond::render::RenderTargetDesc{.targetSnapshot = latestSnapshot});
    if (!targetResult)
    {
        if (auto reason = pond::render::test::MakeOptionalLiveSkipReason(
                targetResult.GetError(), pond::render::test::LivePrerequisiteOperation::Target, "Vulkan render-target creation"))
        {
            GTEST_SKIP() << *reason;
        }
    ...
    const auto duplicateTargetResult =
        device.CreateRenderTarget(std::move(surface), pond::render::RenderTargetDesc{.targetSnapshot = latestSnapshot});
    ASSERT_FALSE(duplicateTargetResult);
    EXPECT_EQ(duplicateTargetResult.GetError().GetCode(), pond::render::ToErrorCode(pond::render::RenderErrorCode::InvalidState));
    EXPECT_EQ(device.GetActiveTargetCount(), 1U);
    EXPECT_EQ(bootstrap.GetActiveTargetCount(), 1U);
    ...
    const std::array<pond::render::ClearColor, 3U> clearSequence{pond::render::ClearColor{.red = 0.9F, .green = 0.1F, .blue = 0.1F, .alpha = 1.0F},
    ...
    const pond::render::RenderTargetSnapshot zeroSizedSnapshot{window.GetId(),
    ...
    const pond::render::RenderTargetSnapshot activeSnapshot{window.GetId(),
    ...
    EXPECT_EQ(target.GetPendingRecreationInfo()->reason, pond::render::TargetRecreationReason::Restored);
    ...
    ASSERT_NO_FATAL_FAILURE(ExpectPresentedFrame(target, pond::render::ClearColor{}, &restoredPresentation));
    EXPECT_TRUE(target.HasSwapchain());
    ...
    const pond::core::VoidResult wrongWindowUpdate = target.UpdateTargetSnapshot(
        pond::render::RenderTargetSnapshot{ponder::platform::WindowId{9999}, latestSnapshot.GetPixelSize(), latestSnapshot.GetLogicalSize(), true,
                                           ponder::platform::WindowState::Normal, pond::render::PresentationEnvironmentRevision{1U}, 5});
    ASSERT_FALSE(wrongWindowUpdate);
    EXPECT_EQ(wrongWindowUpdate.GetError().GetCode(), pond::render::ToErrorCode(pond::render::RenderErrorCode::InvalidArgument));

    const pond::core::VoidResult staleUpdate = target.UpdateTargetSnapshot(activeSnapshot);
    ASSERT_FALSE(staleUpdate);
    EXPECT_EQ(staleUpdate.GetError().GetCode(), pond::render::ToErrorCode(pond::render::RenderErrorCode::InvalidState));
    ...
    {
        ponder::platform::WindowDesc cycleDesc;
        cycleDesc.title = "Ponder Render Repeated Window " + std::to_string(cycle);
        cycleDesc.logicalSize = {240U + static_cast<std::uint32_t>(cycle) * 20U, 180U + static_cast<std::uint32_t>(cycle) * 20U};
        ...
        ponder::platform::Window cycleWindow = std::move(cycleWindowResult).GetValue();
        ...
        const pond::render::RenderTargetSnapshot cycleSnapshot = std::move(cycleSnapshotResult).GetValue();

        auto cycleSurfaceResult = PrepareTransferredSurface(bootstrap, cycleWindow, cycleSnapshot, cycleSnapshot);
        ASSERT_TRUE(cycleSurfaceResult) << cycleSurfaceResult.GetError().GetMessage();
        auto cycleTargetResult =
            device.CreateRenderTarget(std::move(cycleSurfaceResult).GetValue(), pond::render::RenderTargetDesc{.targetSnapshot = cycleSnapshot});
        ...
        ASSERT_NO_FATAL_FAILURE(ExpectPresentedFrame(cycleTarget, MakeClearColor(0.2F, 0.3F, 0.4F)));
    }
    EXPECT_EQ(device.GetActiveTargetCount(), 0U);
    EXPECT_EQ(bootstrap.GetActiveTargetCount(), 0U);
...
TEST(RenderOwnerThreadSurfaceIntegrationTests, KeepsTwoLiveWindowsIndependentOnOneRenderDevice)
{
    ...
    {
        if (auto reason = pond::render::test::MakeOptionalLiveSkipReason(firstWindowResult.GetError(),
    ...
    auto firstWindow = std::make_unique<ponder::platform::Window>(std::move(firstWindowResult).GetValue());

    auto secondWindowResult = runtime.CreateWindow(secondDesc);
    if (!secondWindowResult)
    {
        if (auto reason = pond::render::test::MakeOptionalLiveSkipReason(secondWindowResult.GetError(),
                                                                         pond::render::test::LivePrerequisiteOperation::PlatformWindow,
    ...
    ponder::platform::Window secondWindow = std::move(secondWindowResult).GetValue();
    ...
    auto firstSurfaceResult = PrepareTransferredSurface(bootstrap, *firstWindow, firstInitial, firstActive);
    if (!firstSurfaceResult)
    {
        if (auto reason = pond::render::test::MakeOptionalLiveSkipReason(
                firstSurfaceResult.GetError(), pond::render::test::LivePrerequisiteOperation::Surface, "First Vulkan surface preparation"))
        {
            GTEST_SKIP() << *reason;
        }
    ...
    auto secondSurfaceResult = PrepareTransferredSurface(bootstrap, secondWindow, secondInitial, secondActive);
    if (!secondSurfaceResult)
    {
        if (auto reason = pond::render::test::MakeOptionalLiveSkipReason(
                secondSurfaceResult.GetError(), pond::render::test::LivePrerequisiteOperation::Surface, "Second Vulkan surface preparation"))
        {
            GTEST_SKIP() << *reason;
        }
    ...
    auto selectionResult = bootstrap.SelectAdapter(firstSurface, pond::render::RenderAdapterSelectionDesc{});
    if (!selectionResult)
    {
        if (auto reason = pond::render::test::MakeOptionalLiveSkipReason(
                selectionResult.GetError(), pond::render::test::LivePrerequisiteOperation::Adapter, "Vulkan adapter selection"))
    ...
    auto deviceResult = bootstrap.CreateDevice(firstSurface, selectionResult.GetValue(), pond::render::RenderDeviceDesc{});
    if (!deviceResult)
    {
        if (auto reason = pond::render::test::MakeOptionalLiveSkipReason(
                deviceResult.GetError(), pond::render::test::LivePrerequisiteOperation::Device, "Vulkan logical-device creation"))
    ...
    auto firstTargetResult = device.CreateRenderTarget(std::move(firstSurface), pond::render::RenderTargetDesc{.targetSnapshot = firstActive});
    if (!firstTargetResult)
    {
        if (auto reason = pond::render::test::MakeOptionalLiveSkipReason(
                firstTargetResult.GetError(), pond::render::test::LivePrerequisiteOperation::Target, "First Vulkan render-target creation"))
        {
            GTEST_SKIP() << *reason;
        }
    ...
    auto secondTargetResult = device.CreateRenderTarget(std::move(secondSurface), pond::render::RenderTargetDesc{.targetSnapshot = secondActive});
    if (!secondTargetResult)
    {
        if (auto reason = pond::render::test::MakeOptionalLiveSkipReason(
                secondTargetResult.GetError(), pond::render::test::LivePrerequisiteOperation::Target, "Second Vulkan render-target creation"))
        {
            GTEST_SKIP() << *reason;
        }
    ...
    EXPECT_EQ(secondTarget.GetSelectedPresentationConfig()->pixelExtent, secondActive.GetPixelSize());
    EXPECT_NE(firstTarget.GetSelectedPresentationConfig()->pixelExtent, secondTarget.GetSelectedPresentationConfig()->pixelExtent);
    ...
    EXPECT_EQ(duplicateFirstFrame.GetError().GetCode(), pond::render::ToErrorCode(pond::render::RenderErrorCode::InvalidState));
    const pond::render::RenderTargetDiagnostics firstDiagnosticsAfterMisuse = firstTarget.GetDiagnostics();
    EXPECT_EQ(firstDiagnosticsAfterMisuse.windowId, firstWindow->GetId());
    EXPECT_EQ(firstDiagnosticsAfterMisuse.targetLabel, "target/window:" + std::to_string(firstWindow->GetId().GetValue()));
    ...
    EXPECT_EQ(firstDiagnosticsAfterMisuse.lastFailure->renderCode, pond::render::RenderErrorCode::InvalidState);
    ...
    constexpr std::array<ponder::platform::LogicalSize, 4U> kResizeStorm{
        ponder::platform::LogicalSize{360, 260}, ponder::platform::LogicalSize{420, 300}, ponder::platform::LogicalSize{380, 270},
        ponder::platform::LogicalSize{400, 280}};
    for (const ponder::platform::LogicalSize size : kResizeStorm)
    {
    ...
    ASSERT_TRUE(pond::render::test::PumpPlatformEventsUntil(runtime, firstEvents,
                                                            [&firstWindow]()
                                                            {
                                                                ...
                                                                return logicalSize &&
                                                                       logicalSize.GetValue() == ponder::platform::LogicalSize{400, 280};
                                                            }));
    ...
    const pond::render::RenderTargetSnapshot firstResized = std::move(firstResizedResult).GetValue();
    ...
    EXPECT_EQ(firstTarget.GetPendingRecreationInfo()->reason, pond::render::TargetRecreationReason::SizeChanged);
    ...
    ASSERT_TRUE(firstTarget.GetSelectedPresentationConfig().has_value());
    EXPECT_EQ(firstTarget.GetSelectedPresentationConfig()->pixelExtent, firstResized.GetPixelSize());
    ...
    ASSERT_TRUE(pond::render::test::PumpPlatformEventsUntil(runtime, firstEvents,
                                                            [&firstWindow]()
                                                            {
                                                                const auto visible = firstWindow->IsVisible();
                                                                return visible && !visible.GetValue();
                                                            }));
    ...
    ASSERT_TRUE(pond::render::test::PumpPlatformEventsUntil(runtime, firstEvents,
                                                            [&firstWindow]()
                                                            {
                                                                ...
                                                                return state && state.GetValue() == ponder::platform::WindowState::Minimized;
                                                            }));
    ...
    const pond::render::RenderTargetSnapshot firstMinimized = std::move(firstMinimizedResult).GetValue();
    ...
    const pond::render::PresentationRetirementStats firstRetirement = firstTarget.GetPresentationRetirementStats();
    ...
    ASSERT_NO_FATAL_FAILURE(ExpectPresentedFrame(secondTarget, MakeClearColor(0.0F, 0.8F, 0.8F)));
    const pond::render::RenderTargetDiagnostics firstDiagnosticsAfterMinimize = firstTarget.GetDiagnostics();
    ...
    const pond::render::RenderTargetSnapshot firstRestored = std::move(firstRestoredResult).GetValue();
    ...
    ASSERT_TRUE(pond::render::test::PumpPlatformEventsUntil(runtime, secondEvents,
                                                            [&secondWindow]()
                                                            {
    ...
    const pond::render::RenderTargetSnapshot secondMinimized = std::move(secondMinimizedResult).GetValue();
    ASSERT_TRUE(pond::render::IsValid(secondMinimized));
    const pond::core::VoidResult minimizeSecond = secondTarget.UpdateTargetSnapshot(secondMinimized);
    ...
    const pond::render::PresentationRetirementStats secondRetirement = secondTarget.GetPresentationRetirementStats();
    ...
    const pond::render::RenderTargetSnapshot secondRestored = std::move(secondRestoredResult).GetValue();
...
TEST(RenderOwnerThreadSurfaceIntegrationTests, ShutsDownRendererChildrenOnDedicatedRenderThread)
{
    ...
    {
        if (auto reason = pond::render::test::MakeOptionalLiveSkipReason(
                windowResult.GetError(), pond::render::test::LivePrerequisiteOperation::PlatformWindow, "Dedicated-thread Vulkan window creation"))
        {
            GTEST_SKIP() << *reason;
        }
        FAIL() << windowResult.GetError().GetMessage();
    ...
    {
        if (auto reason = pond::render::test::MakeOptionalLiveSkipReason(
                bootstrapResult.GetError(), pond::render::test::LivePrerequisiteOperation::Bootstrap, "Dedicated-thread render bootstrap creation"))
        {
            GTEST_SKIP() << *reason;
        }
        FAIL() << bootstrapResult.GetError().GetMessage();
    ...
    pond::render::RenderBootstrap bootstrap = std::move(bootstrapResult).GetValue();
    auto surfaceResult = bootstrap.PrepareSurface(
        window, pond::render::SurfacePreparationDesc{.targetSnapshot = snapshot, .reason = pond::render::SurfacePreparationReason::Initial});
    if (!surfaceResult)
    {
        if (auto reason = pond::render::test::MakeOptionalLiveSkipReason(
                surfaceResult.GetError(), pond::render::test::LivePrerequisiteOperation::Surface, "Dedicated-thread Vulkan surface preparation"))
        {
            GTEST_SKIP() << *reason;
        }
        FAIL() << surfaceResult.GetError().GetMessage();
    ...
    std::thread renderThread{
        [&bootstrap, snapshot, surface = std::move(surface), &outcome]() mutable
        {
            const auto recordFailure =
                [&outcome](const pond::core::Error& error, pond::render::test::LivePrerequisiteOperation operation, std::string_view context)
            {
                outcome.optionalSkip = pond::render::test::MakeOptionalLiveSkipReason(error, operation, context);
            ...
            if (pond::core::VoidResult transfer = surface.TransferToCurrentThread(snapshot); !transfer)
            {
                recordFailure(transfer.GetError(), pond::render::test::LivePrerequisiteOperation::Surface, "Prepared-surface render-thread transfer");
                return;
            }

            auto selection = bootstrap.SelectAdapter(surface, pond::render::RenderAdapterSelectionDesc{});
            if (!selection)
            {
                recordFailure(selection.GetError(), pond::render::test::LivePrerequisiteOperation::Adapter, "Dedicated-thread adapter selection");
                return;
            }

            auto deviceResult = bootstrap.CreateDevice(surface, selection.GetValue(), pond::render::RenderDeviceDesc{});
            if (!deviceResult)
            {
                recordFailure(deviceResult.GetError(), pond::render::test::LivePrerequisiteOperation::Device, "Dedicated-thread device creation");
                return;
            }
            pond::render::RenderDevice device = std::move(deviceResult).GetValue();

            auto targetResult = device.CreateRenderTarget(std::move(surface), pond::render::RenderTargetDesc{.targetSnapshot = snapshot});
            if (!targetResult)
            {
                recordFailure(targetResult.GetError(), pond::render::test::LivePrerequisiteOperation::Target, "Dedicated-thread target creation");
                return;
            }
            pond::render::RenderTarget target = std::move(targetResult).GetValue();
            ...
            for (std::size_t attempt = 0U; attempt < kMaximumAttempts; ++attempt)
            {
                ...
                if (!frameResult)
                {
                    outcome.failure = "Dedicated-thread frame acquisition: " + std::string{frameResult.GetError().GetMessage()};
                    return;
                }
                pond::render::RenderFrame frame = std::move(frameResult).GetValue();
                if (pond::core::VoidResult clear = frame.Clear(MakeClearColor(0.2F, 0.5F, 0.8F)); !clear)
                {
                    outcome.failure = "Dedicated-thread clear: " + std::string{clear.GetError().GetMessage()};
                    return;
                }
                auto presentation = frame.FinishAndPresent();
                if (!presentation)
                {
                    outcome.failure = "Dedicated-thread presentation: " + std::string{presentation.GetError().GetMessage()};
                    return;
                }
