...
static_assert(kUnknownLogicalKey.GetKind() ==
...
static_assert(pond::platform::LogicalKey::FromCharacter(U'\0').GetKind() ==
              pond::platform::LogicalKey::Kind::Unknown);
static_assert(pond::platform::LogicalKey::FromCharacter(
...
{
    EXPECT_EQ(pond::platform::PhysicalKey{},
    ...
    const auto forged = static_cast<pond::platform::NamedKey>(
        std::numeric_limits<std::uint16_t>::max());
    EXPECT_EQ(pond::platform::LogicalKey::FromNamed(forged),
...
TEST(KeyboardTests, CombinesAndQueriesIndependentModifierBits)
{
    ...
    EXPECT_TRUE(pond::platform::HasAnyKeyModifier(modifiers,
                                                  KeyModifiers::Control));
    EXPECT_TRUE(pond::platform::HasAnyKeyModifier(modifiers,
                                                  KeyModifiers::Shift));
    EXPECT_TRUE(pond::platform::HasAllKeyModifiers(
        modifiers, KeyModifiers::LeftControl | KeyModifiers::RightShift));
    EXPECT_FALSE(pond::platform::HasAnyKeyModifier(modifiers,
                                                   KeyModifiers::Alt));
    EXPECT_FALSE(pond::platform::HasAllKeyModifiers(modifiers,
                                                    KeyModifiers::Shift));
