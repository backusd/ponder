...
static_assert(
    std::is_same_v<decltype(pond::platform::NativeWin32Window::instance), void*>);
static_assert(
    std::is_same_v<decltype(pond::platform::NativeWin32Window::window), void*>);
static_assert(
    std::is_same_v<decltype(pond::platform::NativeX11Window::display), void*>);
static_assert(std::is_same_v<decltype(pond::platform::NativeX11Window::window),
                             std::uintptr_t>);
static_assert(
    std::is_same_v<decltype(pond::platform::NativeWaylandWindow::display), void*>);
static_assert(
    std::is_same_v<decltype(pond::platform::NativeWaylandWindow::surface), void*>);
static_assert(
    std::is_same_v<decltype(pond::platform::NativeCocoaWindow::metalLayer), void*>);
...
TEST(NativeWindowHandleTests, IsAClosedTaggedVariant)
{
    ...
    const pond::platform::NativeWindowHandle x11 =
        pond::platform::NativeX11Window{reinterpret_cast<void*>(0x3000),
                                        std::uintptr_t{0x4000}};
    ASSERT_TRUE(std::holds_alternative<pond::platform::NativeX11Window>(x11));
    EXPECT_EQ(std::get<pond::platform::NativeX11Window>(x11).window,
              std::uintptr_t{0x4000});
