...
constexpr ponder::platform::DisplayId kDisplayId{11};
constexpr std::uint64_t kTimestampNanoseconds{123'456'789};

[[nodiscard]] constexpr ponder::core::Timestamp MakeTimestamp(std::int64_t nanoseconds = static_cast<std::int64_t>(kTimestampNanoseconds))
{
    return ponder::core::Timestamp{std::chrono::nanoseconds{nanoseconds}};
}

struct ResolverState final
{
    std::unordered_map<std::uint32_t, ponder::platform::WindowId> windows;
    std::unordered_map<std::uint32_t, ponder::platform::DisplayId> displays;
    std::vector<std::uint32_t> windowRequests;
    std::vector<std::uint32_t> displayRequests;
    bool throwDisplayResolutionFailure{};
};

[[nodiscard]] std::optional<ponder::platform::WindowId> ResolveWindowId(void* context, std::uint32_t backendWindowId)
{
    ...
    return iterator == state.windows.end() ? std::nullopt : std::optional<ponder::platform::WindowId>{iterator->second};
}

[[nodiscard]] std::optional<ponder::platform::DisplayId> ResolveDisplayId(void* context, std::uint32_t backendDisplayId)
{
    ResolverState& state = *static_cast<ResolverState*>(context);
    state.displayRequests.push_back(backendDisplayId);
    if (state.throwDisplayResolutionFailure)
    {
        throw PLATFORM_EXCEPTION(ponder::platform::PlatformErrorCode::BackendFailure, "synthetic display resolver failure");
    }

    const auto iterator = state.displays.find(backendDisplayId);
    return iterator == state.displays.end() ? std::nullopt : std::optional<ponder::platform::DisplayId>{iterator->second};
}
...
[[nodiscard]] SDL_Event MakeWindowEvent(SDL_EventType type, std::uint64_t timestamp = kTimestampNanoseconds,
...
[[nodiscard]] SDL_Event MakeDisplayEvent(SDL_EventType type, std::uint64_t timestamp = kTimestampNanoseconds,
...
[[nodiscard]] SDL_Event MakeKeyboardEvent(SDL_EventType type, SDL_Scancode scancode, SDL_Keycode key, SDL_Keymod modifiers = SDL_KMOD_NONE,
                                          bool repeat = false, std::uint32_t backendWindowId = kBackendWindowId,
...
[[nodiscard]] SDL_Event MakeMouseMotionEvent(float x = 12.25F, float y = -8.5F, float relativeX = 3.75F, float relativeY = -1.5F,
                                             std::uint32_t backendWindowId = kBackendWindowId, std::uint64_t timestamp = kTimestampNanoseconds,
                                             SDL_MouseID which = 0)
{
    SDL_Event event{};
...
[[nodiscard]] SDL_Event MakeMouseButtonEvent(SDL_EventType type, std::uint8_t button = SDL_BUTTON_LEFT, float x = 12.25F, float y = -8.5F,
                                             std::uint32_t backendWindowId = kBackendWindowId, std::uint64_t timestamp = kTimestampNanoseconds,
                                             SDL_MouseID which = 0)
{
    SDL_Event event{};
...
[[nodiscard]] SDL_Event MakeMouseWheelEvent(float horizontal = 1.25F, float vertical = -2.5F,
...
[[nodiscard]] SDL_Event MakeTextInputEvent(const char* text, std::uint32_t backendWindowId = kBackendWindowId,
                                           std::uint64_t timestamp = kTimestampNanoseconds)
{
    SDL_Event event{};
...
[[nodiscard]] SDL_Event MakeTextCompositionEvent(const char* text, std::int32_t start, std::int32_t length,
                                                 std::uint32_t backendWindowId = kBackendWindowId, std::uint64_t timestamp = kTimestampNanoseconds)
{
    SDL_Event event{};
...
[[nodiscard]] SDL_Event MakeDropEvent(SDL_EventType type, const char* data = nullptr, const char* source = nullptr, float x = 12.25F, float y = -8.5F,
                                      std::uint32_t backendWindowId = kBackendWindowId, std::uint64_t timestamp = kTimestampNanoseconds)
{
    SDL_Event event{};
...
void ExpectTranslatedEvent(const std::optional<ponder::platform::PlatformEvent>& translated, const Event& expected)
{
...
protected:
    ...
    ponder::platform::detail::EventTranslationContext m_context;
};

TEST_F(SdlEventTranslatorTests, TranslatesQuitAndEveryWindowLifecycleSignal)
{
    const ponder::core::Timestamp timestamp = MakeTimestamp();

    ExpectTranslatedEvent(Translate(MakeQuitEvent()), ponder::platform::QuitRequestedEvent{.timestamp = timestamp});
    EXPECT_TRUE(m_resolvers.windowRequests.empty());
    EXPECT_TRUE(m_resolvers.displayRequests.empty());

    ExpectTranslatedEvent(Translate(MakeWindowEvent(SDL_EVENT_WINDOW_CLOSE_REQUESTED)),
                          ponder::platform::WindowCloseRequestedEvent{.timestamp = timestamp, .windowId = kWindowId});
    ExpectTranslatedEvent(Translate(MakeWindowEvent(SDL_EVENT_WINDOW_FOCUS_GAINED)),
                          ponder::platform::WindowFocusChangedEvent{.timestamp = timestamp, .windowId = kWindowId, .focused = true});
    ExpectTranslatedEvent(Translate(MakeWindowEvent(SDL_EVENT_WINDOW_FOCUS_LOST)),
                          ponder::platform::WindowFocusChangedEvent{.timestamp = timestamp, .windowId = kWindowId, .focused = false});
    ExpectTranslatedEvent(Translate(MakeWindowEvent(SDL_EVENT_WINDOW_SHOWN)),
                          ponder::platform::WindowVisibilityChangedEvent{.timestamp = timestamp, .windowId = kWindowId, .visible = true});
    ExpectTranslatedEvent(Translate(MakeWindowEvent(SDL_EVENT_WINDOW_HIDDEN)),
                          ponder::platform::WindowVisibilityChangedEvent{.timestamp = timestamp, .windowId = kWindowId, .visible = false});
    ExpectTranslatedEvent(
        Translate(MakeWindowEvent(SDL_EVENT_WINDOW_MINIMIZED)),
        ponder::platform::WindowStateChangedEvent{.timestamp = timestamp, .windowId = kWindowId, .state = ponder::platform::WindowState::Minimized});
    ExpectTranslatedEvent(
        Translate(MakeWindowEvent(SDL_EVENT_WINDOW_MAXIMIZED)),
        ponder::platform::WindowStateChangedEvent{.timestamp = timestamp, .windowId = kWindowId, .state = ponder::platform::WindowState::Maximized});
    ExpectTranslatedEvent(
        Translate(MakeWindowEvent(SDL_EVENT_WINDOW_RESTORED)),
        ponder::platform::WindowStateChangedEvent{.timestamp = timestamp, .windowId = kWindowId, .state = ponder::platform::WindowState::Normal});
    ExpectTranslatedEvent(Translate(MakeWindowEvent(SDL_EVENT_WINDOW_ENTER_FULLSCREEN)),
                          ponder::platform::WindowPresentationChangedEvent{.timestamp = timestamp,
                                                                           .windowId = kWindowId,
                                                                           .presentation = ponder::platform::WindowPresentation::DesktopFullscreen});
    ExpectTranslatedEvent(Translate(MakeWindowEvent(SDL_EVENT_WINDOW_LEAVE_FULLSCREEN)),
                          ponder::platform::WindowPresentationChangedEvent{.timestamp = timestamp,
                                                                           .windowId = kWindowId,
                                                                           .presentation = ponder::platform::WindowPresentation::Windowed});
    ExpectTranslatedEvent(Translate(MakeWindowEvent(SDL_EVENT_WINDOW_DISPLAY_SCALE_CHANGED)),
                          ponder::platform::WindowDisplayScaleChangedEvent{.timestamp = timestamp, .windowId = kWindowId});
    ExpectTranslatedEvent(Translate(MakeWindowEvent(SDL_EVENT_WINDOW_MOUSE_ENTER)),
                          ponder::platform::WindowPointerEnteredEvent{.timestamp = timestamp, .windowId = kWindowId});
    ExpectTranslatedEvent(Translate(MakeWindowEvent(SDL_EVENT_WINDOW_MOUSE_LEAVE)),
                          ponder::platform::WindowPointerLeftEvent{.timestamp = timestamp, .windowId = kWindowId});
}

TEST_F(SdlEventTranslatorTests, TranslatesKnownPhysicalUnicodeAndEveryModifierBit)
{
    constexpr SDL_Keymod kAllModifiers = static_cast<SDL_Keymod>(SDL_KMOD_LSHIFT | SDL_KMOD_RSHIFT | SDL_KMOD_LEVEL5 | SDL_KMOD_LCTRL |
                                                                 SDL_KMOD_RCTRL | SDL_KMOD_LALT | SDL_KMOD_RALT | SDL_KMOD_LGUI | SDL_KMOD_RGUI |
                                                                 SDL_KMOD_NUM | SDL_KMOD_CAPS | SDL_KMOD_MODE | SDL_KMOD_SCROLL | 0x0008U);
    constexpr ponder::platform::KeyModifiers kExpectedModifiers =
        ponder::platform::KeyModifiers::LeftShift | ponder::platform::KeyModifiers::RightShift | ponder::platform::KeyModifiers::Level5Shift |
        ponder::platform::KeyModifiers::LeftControl | ponder::platform::KeyModifiers::RightControl | ponder::platform::KeyModifiers::LeftAlt |
        ponder::platform::KeyModifiers::RightAlt | ponder::platform::KeyModifiers::LeftSuper | ponder::platform::KeyModifiers::RightSuper |
        ponder::platform::KeyModifiers::NumLock | ponder::platform::KeyModifiers::CapsLock | ponder::platform::KeyModifiers::AltGraph |
        ponder::platform::KeyModifiers::ScrollLock;

    SDL_Event event = MakeKeyboardEvent(SDL_EVENT_KEY_DOWN, SDL_SCANCODE_Q, static_cast<SDL_Keycode>(U'\u00E9'), kAllModifiers, true);
    event.key.raw = 0xFFFFU;

    ExpectTranslatedEvent(Translate(event), ponder::platform::KeyboardKeyEvent{.timestamp = MakeTimestamp(),
                                                                               .windowId = kWindowId,
                                                                               .physicalKey = ponder::platform::PhysicalKey::Q,
                                                                               .logicalKey = ponder::platform::LogicalKey::FromCharacter(U'\u00E9'),
...
TEST_F(SdlEventTranslatorTests, TranslatesNamedAndUnknownKeyValues)
{
    ExpectTranslatedEvent(
        ...
        ponder::platform::KeyboardKeyEvent{.timestamp = MakeTimestamp(),
                                           .windowId = kWindowId,
                                           .physicalKey = ponder::platform::PhysicalKey::Keypad7,
                                           .logicalKey = ponder::platform::LogicalKey::FromNamed(ponder::platform::NamedKey::Keypad7),
                                           .modifiers = ponder::platform::KeyModifiers::None,
                                           .pressed = true,
                                           .repeat = false});

    ExpectTranslatedEvent(Translate(MakeKeyboardEvent(SDL_EVENT_KEY_DOWN, static_cast<SDL_Scancode>(0x7FFF), SDLK_UNKNOWN)),
                          ponder::platform::KeyboardKeyEvent{.timestamp = MakeTimestamp(),
                                                             .windowId = kWindowId,
                                                             .physicalKey = ponder::platform::PhysicalKey::Unknown,
                                                             .logicalKey = ponder::platform::LogicalKey::Unknown(),
                                                             .modifiers = ponder::platform::KeyModifiers::None,
                                                             .pressed = true,
                                                             .repeat = false});

    ExpectTranslatedEvent(Translate(MakeKeyboardEvent(SDL_EVENT_KEY_DOWN, SDL_SCANCODE_A, static_cast<SDL_Keycode>(0x0011'0000U))),
                          ponder::platform::KeyboardKeyEvent{.timestamp = MakeTimestamp(),
                                                             .windowId = kWindowId,
                                                             .physicalKey = ponder::platform::PhysicalKey::A,
                                                             .logicalKey = ponder::platform::LogicalKey::Unknown(),
                                                             .modifiers = ponder::platform::KeyModifiers::None,
                                                             .pressed = true,
                                                             .repeat = false});
...
{
    ExpectTranslatedEvent(
        Translate(MakeKeyboardEvent(SDL_EVENT_KEY_UP, SDL_SCANCODE_LEFT, SDLK_LEFT, SDL_KMOD_LCTRL)),
        ponder::platform::KeyboardKeyEvent{.timestamp = MakeTimestamp(),
                                           .windowId = kWindowId,
                                           .physicalKey = ponder::platform::PhysicalKey::ArrowLeft,
                                           .logicalKey = ponder::platform::LogicalKey::FromNamed(ponder::platform::NamedKey::ArrowLeft),
...
TEST_F(SdlEventTranslatorTests, TranslatesFiniteMouseMotionFromMouseTouchAndPenSources)
{
    ...
    {
        ExpectTranslatedEvent(Translate(MakeMouseMotionEvent(-27.5F, 14.25F, 3.5F, -6.75F, kBackendWindowId, kTimestampNanoseconds, source)),
                              ponder::platform::MouseMotionEvent{.timestamp = MakeTimestamp(),
                                                                 .windowId = kWindowId,
...
TEST_F(SdlEventTranslatorTests, MapsEveryMouseButtonAndEnforcesPressStateConsistency)
{
    ...
    {
        std::uint8_t backend;
        ponder::platform::MouseButton expected;
    };

    constexpr std::array<ButtonCase, 6> kButtonCases{{
        {SDL_BUTTON_LEFT, ponder::platform::MouseButton::Left},
        {SDL_BUTTON_RIGHT, ponder::platform::MouseButton::Right},
        {SDL_BUTTON_MIDDLE, ponder::platform::MouseButton::Middle},
        {SDL_BUTTON_X1, ponder::platform::MouseButton::X1},
        {SDL_BUTTON_X2, ponder::platform::MouseButton::X2},
        {0xFFU, ponder::platform::MouseButton::Unknown},
    }};

    for (const ButtonCase& buttonCase : kButtonCases)
    {
        ExpectTranslatedEvent(Translate(MakeMouseButtonEvent(SDL_EVENT_MOUSE_BUTTON_DOWN, buttonCase.backend, -9.5F, 4.25F, kBackendWindowId,
                                                             kTimestampNanoseconds, SDL_PEN_MOUSEID)),
                              ponder::platform::MouseButtonEvent{.timestamp = MakeTimestamp(),
                                                                 .windowId = kWindowId,
                                                                 .position = {-9.5F, 4.25F},
    ...
    ExpectTranslatedEvent(Translate(MakeMouseButtonEvent(SDL_EVENT_MOUSE_BUTTON_UP, SDL_BUTTON_RIGHT, 5.5F, -2.25F, kBackendWindowId,
                                                         kTimestampNanoseconds, SDL_TOUCH_MOUSEID)),
                          ponder::platform::MouseButtonEvent{.timestamp = MakeTimestamp(),
                                                             .windowId = kWindowId,
                                                             .position = {5.5F, -2.25F},
                                                             .button = ponder::platform::MouseButton::Right,
                                                             .pressed = false});
...
{
    ExpectTranslatedEvent(Translate(MakeMouseWheelEvent(2.25F, -3.5F, SDL_MOUSEWHEEL_NORMAL, -8.75F, 11.5F, kBackendWindowId, kTimestampNanoseconds,
                                                        SDL_TOUCH_MOUSEID)),
                          ponder::platform::MouseWheelEvent{.timestamp = MakeTimestamp(),
                                                            .windowId = kWindowId,
    ...
    ExpectTranslatedEvent(
        Translate(MakeMouseWheelEvent(4.5F, -6.25F, SDL_MOUSEWHEEL_FLIPPED, 7.0F, -12.0F, kBackendWindowId, kTimestampNanoseconds, SDL_PEN_MOUSEID)),
        ponder::platform::MouseWheelEvent{.timestamp = MakeTimestamp(),
                                          .windowId = kWindowId,
                                          .position = {7.0F, -12.0F},
                                          .horizontal = -4.5F,
    ...
    EXPECT_FALSE(Translate(MakeMouseWheelEvent(1.0F, 1.0F, static_cast<SDL_MouseWheelDirection>(0x7FFF))).has_value());
}
...
TEST_F(SdlEventTranslatorTests, PreservesTargetlessInputAndDropsStaleNonzeroTargets)
{
    ...
    auto key = Translate(MakeKeyboardEvent(SDL_EVENT_KEY_DOWN, SDL_SCANCODE_A, SDLK_A, SDL_KMOD_NONE, false, 0));
    ASSERT_TRUE(key.has_value());
    ASSERT_TRUE(std::holds_alternative<ponder::platform::KeyboardKeyEvent>(*key));
    EXPECT_FALSE(std::get<ponder::platform::KeyboardKeyEvent>(*key).windowId.has_value());

    auto text = Translate(MakeTextInputEvent("targetless", 0));
    ASSERT_TRUE(text.has_value());
    ASSERT_TRUE(std::holds_alternative<ponder::platform::TextInputEvent>(*text));
    EXPECT_FALSE(std::get<ponder::platform::TextInputEvent>(*text).windowId.has_value());

    auto composition = Translate(MakeTextCompositionEvent("targetless", 0, 0, 0));
    ASSERT_TRUE(composition.has_value());
    ASSERT_TRUE(std::holds_alternative<ponder::platform::TextCompositionEvent>(*composition));
    EXPECT_FALSE(std::get<ponder::platform::TextCompositionEvent>(*composition).windowId.has_value());
    ...
    ASSERT_TRUE(std::holds_alternative<ponder::platform::MouseMotionEvent>(*motion));
    EXPECT_FALSE(std::get<ponder::platform::MouseMotionEvent>(*motion).windowId.has_value());
    ...
    ASSERT_TRUE(std::holds_alternative<ponder::platform::MouseButtonEvent>(*button));
    EXPECT_FALSE(std::get<ponder::platform::MouseButtonEvent>(*button).windowId.has_value());
    ...
    ASSERT_TRUE(std::holds_alternative<ponder::platform::MouseWheelEvent>(*wheel));
    EXPECT_FALSE(std::get<ponder::platform::MouseWheelEvent>(*wheel).windowId.has_value());
    EXPECT_TRUE(m_resolvers.windowRequests.empty());

    EXPECT_FALSE(Translate(MakeKeyboardEvent(SDL_EVENT_KEY_DOWN, SDL_SCANCODE_A, SDLK_A, SDL_KMOD_NONE, false, 999)).has_value());
    ...
    EXPECT_EQ(m_resolvers.windowRequests, (std::vector<std::uint32_t>{999, 999, 999, 999, 999, 999}));
}
...
TEST_F(SdlEventTranslatorTests, OwnsAndValidatesUtf8TextInput)
{
    ...
    ExpectTranslatedEvent(translated, ponder::platform::TextInputEvent{.timestamp = MakeTimestamp(), .windowId = kWindowId, .text = "caf\xC3\xA9"});
    ExpectTranslatedEvent(Translate(MakeTextInputEvent("")),
                          ponder::platform::TextInputEvent{.timestamp = MakeTimestamp(), .windowId = kWindowId, .text = ""});
...
TEST_F(SdlEventTranslatorTests, TranslatesAndOwnsEveryDropEvent)
{
    ...
    ExpectTranslatedEvent(begin, ponder::platform::DropBeginEvent{.timestamp = MakeTimestamp(100),
                                                                  .windowId = kWindowId,
                                                                  .sourceApplication = std::optional<std::string>{"source-app"}});
    ExpectTranslatedEvent(position, ponder::platform::DropPositionEvent{.timestamp = MakeTimestamp(200),
                                                                        .windowId = kWindowId,
                                                                        .position = {-4.5F, 8.25F},
                                                                        .sourceApplication = std::optional<std::string>{"source-app"}});
    ExpectTranslatedEvent(file, ponder::platform::DroppedFileEvent{.timestamp = MakeTimestamp(300),
                                                                   .windowId = kWindowId,
                                                                   .path = std::filesystem::path{"C:/tmp/molecule.sdf"},
                                                                   .position = {1.5F, 2.25F},
                                                                   .sourceApplication = std::optional<std::string>{"source-app"}});
    ExpectTranslatedEvent(droppedText, ponder::platform::DroppedTextEvent{.timestamp = MakeTimestamp(400),
                                                                          .windowId = kWindowId,
                                                                          .text = std::string{"ligand \xF0\x9F\xA7\xAA", 11},
                                                                          .position = {3.5F, 4.25F},
                                                                          .sourceApplication = std::optional<std::string>{"source-app"}});
    ExpectTranslatedEvent(complete, ponder::platform::DropCompleteEvent{.timestamp = MakeTimestamp(500),
                                                                        .windowId = kWindowId,
                                                                        .position = {5.5F, 6.25F},
                                                                        .sourceApplication = std::optional<std::string>{"source-app"}});
}
...
TEST_F(SdlEventTranslatorTests, PreservesTargetlessDropsAndDoesNotRequireSequenceState)
{
    ...
    ExpectTranslatedEvent(
        ...
        ponder::platform::DropBeginEvent{.timestamp = MakeTimestamp(), .windowId = std::nullopt, .sourceApplication = std::nullopt});
    ExpectTranslatedEvent(Translate(MakeDropEvent(SDL_EVENT_DROP_COMPLETE, nullptr, nullptr, 7.5F, 9.25F, 0)),
                          ponder::platform::DropCompleteEvent{.timestamp = MakeTimestamp(),
                                                              .windowId = std::nullopt,
                                                              .position = {7.5F, 9.25F},
                                                              .sourceApplication = std::nullopt});
    ExpectTranslatedEvent(Translate(MakeDropEvent(SDL_EVENT_DROP_FILE, "targetless.sdf", nullptr, 1.0F, 2.0F, 0)),
                          ponder::platform::DroppedFileEvent{.timestamp = MakeTimestamp(),
                                                             .windowId = std::nullopt,
    ...
    EXPECT_FALSE(Translate(MakeDropEvent(SDL_EVENT_DROP_TEXT, "stale", nullptr, 1.0F, 2.0F, kUnknownBackendWindowId)).has_value());
...
TEST_F(SdlEventTranslatorTests, RejectsMalformedDropPayloadsBeforeResolvingTargets)
{
    ...
    EXPECT_FALSE(Translate(MakeDropEvent(SDL_EVENT_DROP_POSITION, nullptr, invalidUtf8)).has_value());
    EXPECT_FALSE(Translate(MakeDropEvent(SDL_EVENT_DROP_POSITION, nullptr, nullptr, nan, 1.0F)).has_value());
    EXPECT_FALSE(Translate(MakeDropEvent(SDL_EVENT_DROP_COMPLETE, nullptr, nullptr, 1.0F, infinity)).has_value());
    EXPECT_TRUE(m_resolvers.windowRequests.empty());
}
...
TEST_F(SdlEventTranslatorTests, OwnsCompositionTextAndDistinguishesSelectionAvailability)
{
    ...
    ASSERT_TRUE(translated.has_value());
    source.assign("overwritten");
    ExpectTranslatedEvent(translated, ponder::platform::TextCompositionEvent{.timestamp = MakeTimestamp(),
                                                                             .windowId = kWindowId,
                                                                             .text = "composition",
                                                                             .selection = ponder::platform::TextCompositionRange{3, 2}});

    ExpectTranslatedEvent(Translate(MakeTextCompositionEvent("", 0, 0)),
                          ponder::platform::TextCompositionEvent{.timestamp = MakeTimestamp(),
                                                                 .windowId = kWindowId,
                                                                 .text = "",
                                                                 .selection = ponder::platform::TextCompositionRange{0, 0}});

    constexpr std::array<std::pair<std::int32_t, std::int32_t>, 4> kUnavailableRanges{{{-1, -1}, {-1, 0}, {0, -1}, {-2, 4}}};
...
TEST_F(SdlEventTranslatorTests, PreservesSignedMovementAndDistinctSizeUnits)
{
    ...
    ExpectTranslatedEvent(Translate(moved), ponder::platform::WindowMovedEvent{
                                                .timestamp = MakeTimestamp(),
                                                .windowId = kWindowId,
                                                .position = {std::numeric_limits<std::int32_t>::min(), std::numeric_limits<std::int32_t>::max()}});
    ...
    ExpectTranslatedEvent(
        Translate(logical),
        ponder::platform::WindowLogicalSizeChangedEvent{.timestamp = MakeTimestamp(), .windowId = kWindowId, .logicalSize = {640, 480}});
    ...
    ExpectTranslatedEvent(
        Translate(pixel),
        ponder::platform::WindowPixelSizeChangedEvent{.timestamp = MakeTimestamp(), .windowId = kWindowId, .pixelSize = {1280, 960}});

    logical.window.data1 = 0;
    logical.window.data2 = 0;
    ExpectTranslatedEvent(
        Translate(logical),
        ponder::platform::WindowLogicalSizeChangedEvent{.timestamp = MakeTimestamp(), .windowId = kWindowId, .logicalSize = {0, 0}});
    pixel.window.data1 = 0;
    pixel.window.data2 = 0;
    ExpectTranslatedEvent(Translate(pixel),
                          ponder::platform::WindowPixelSizeChangedEvent{.timestamp = MakeTimestamp(), .windowId = kWindowId, .pixelSize = {0, 0}});
...
TEST_F(SdlEventTranslatorTests, MapsOptionalWindowDestinationDisplays)
{
    ...
    ExpectTranslatedEvent(Translate(event),
                          ponder::platform::WindowDisplayChangedEvent{.timestamp = MakeTimestamp(), .windowId = kWindowId, .displayId = kDisplayId});
    ...
    ExpectTranslatedEvent(
        Translate(event),
        ponder::platform::WindowDisplayChangedEvent{.timestamp = MakeTimestamp(), .windowId = kWindowId, .displayId = std::nullopt});
    EXPECT_TRUE(m_resolvers.displayRequests.empty());
    ...
    constexpr ponder::platform::DisplayId kHighBitDisplayId{99};
    ...
    ExpectTranslatedEvent(
        Translate(event),
        ponder::platform::WindowDisplayChangedEvent{.timestamp = MakeTimestamp(), .windowId = kWindowId, .displayId = kHighBitDisplayId});
    ASSERT_EQ(m_resolvers.displayRequests.size(), 1U);
    ...
    m_resolvers.displays.emplace(kInvalidMappedDisplayBackendId, ponder::platform::DisplayId::Invalid());
...
TEST_F(SdlEventTranslatorTests, PropagatesResolverFailureForRequiredDisplayEvent)
{
    m_resolvers.throwDisplayResolutionFailure = true;

    try
    {
        static_cast<void>(Translate(MakeDisplayEvent(SDL_EVENT_DISPLAY_MOVED)));
        FAIL() << "Expected ponder::core::Exception";
    }
    catch (const ponder::core::Exception& exception)
    {
        EXPECT_EQ(exception.GetMessage(), "Platform error [backend_failure]: synthetic display resolver failure");
    }
...
TEST_F(SdlEventTranslatorTests, PropagatesResolverFailureForWindowDisplayChange)
{
    SDL_Event event = MakeWindowEvent(SDL_EVENT_WINDOW_DISPLAY_CHANGED);
    event.window.data1 = static_cast<Sint32>(kBackendDisplayId);
    m_resolvers.throwDisplayResolutionFailure = true;
    ...
    try
    {
        static_cast<void>(Translate(event));
        FAIL() << "Expected ponder::core::Exception";
    }
    catch (const ponder::core::Exception& exception)
    {
...
TEST_F(SdlEventTranslatorTests, TranslatesEveryDisplaySignalAndOptionalModeExtent)
{
    ...
    ExpectTranslatedEvent(Translate(MakeDisplayEvent(SDL_EVENT_DISPLAY_ADDED)),
                          ponder::platform::DisplayAddedEvent{.timestamp = timestamp, .displayId = kDisplayId});
    ExpectTranslatedEvent(Translate(MakeDisplayEvent(SDL_EVENT_DISPLAY_REMOVED)),
                          ponder::platform::DisplayRemovedEvent{.timestamp = timestamp, .displayId = kDisplayId});
    ExpectTranslatedEvent(Translate(MakeDisplayEvent(SDL_EVENT_DISPLAY_MOVED)),
                          ponder::platform::DisplayMovedEvent{.timestamp = timestamp, .displayId = kDisplayId});
    ExpectTranslatedEvent(Translate(MakeDisplayEvent(SDL_EVENT_DISPLAY_CONTENT_SCALE_CHANGED)),
                          ponder::platform::DisplayContentScaleChangedEvent{.timestamp = timestamp, .displayId = kDisplayId});
    ExpectTranslatedEvent(Translate(MakeDisplayEvent(SDL_EVENT_DISPLAY_USABLE_BOUNDS_CHANGED)),
                          ponder::platform::DisplayUsableBoundsChangedEvent{.timestamp = timestamp, .displayId = kDisplayId});
    ...
    ExpectTranslatedEvent(Translate(desktopMode),
                          ponder::platform::DisplayDesktopModeChangedEvent{.timestamp = timestamp,
                                                                           .displayId = kDisplayId,
                                                                           .extent = ponder::platform::ScreenExtent{1920, 1080}});
    ...
    ExpectTranslatedEvent(Translate(currentMode),
                          ponder::platform::DisplayCurrentModeChangedEvent{.timestamp = timestamp,
                                                                           .displayId = kDisplayId,
                                                                           .extent = ponder::platform::ScreenExtent{3840, 2160}});

    desktopMode.display.data1 = 0;
    desktopMode.display.data2 = 1080;
    ExpectTranslatedEvent(Translate(desktopMode),
                          ponder::platform::DisplayDesktopModeChangedEvent{.timestamp = timestamp, .displayId = kDisplayId, .extent = std::nullopt});
    desktopMode.display.data1 = 1920;
    ...
    currentMode.display.data2 = 2160;
    ExpectTranslatedEvent(Translate(currentMode),
                          ponder::platform::DisplayCurrentModeChangedEvent{.timestamp = timestamp, .displayId = kDisplayId, .extent = std::nullopt});
    currentMode.display.data1 = 3840;
...
TEST_F(SdlEventTranslatorTests, MapsEveryDisplayOrientationIncludingUnknownValues)
{
    ...
    {
        std::int32_t backend;
        ponder::platform::DisplayOrientation expected;
    };
    constexpr std::array<OrientationCase, 6> kCases{{
        {SDL_ORIENTATION_UNKNOWN, ponder::platform::DisplayOrientation::Unknown},
        {SDL_ORIENTATION_LANDSCAPE, ponder::platform::DisplayOrientation::Landscape},
        {SDL_ORIENTATION_LANDSCAPE_FLIPPED, ponder::platform::DisplayOrientation::LandscapeFlipped},
        {SDL_ORIENTATION_PORTRAIT, ponder::platform::DisplayOrientation::Portrait},
        {SDL_ORIENTATION_PORTRAIT_FLIPPED, ponder::platform::DisplayOrientation::PortraitFlipped},
        {0x7FFF, ponder::platform::DisplayOrientation::Unknown},
    }};
    ...
    for (const OrientationCase& orientationCase : kCases)
    {
        ...
        ExpectTranslatedEvent(Translate(event), ponder::platform::DisplayOrientationChangedEvent{.timestamp = MakeTimestamp(),
                                                                                                 .displayId = kDisplayId,
...
{
    m_resolvers.windowRequests.clear();
    EXPECT_FALSE(Translate(MakeWindowEvent(SDL_EVENT_WINDOW_CLOSE_REQUESTED, kTimestampNanoseconds, 0)).has_value());
    EXPECT_TRUE(m_resolvers.windowRequests.empty());

    constexpr std::uint32_t kUnknownBackendWindowId{42};
    EXPECT_FALSE(Translate(MakeWindowEvent(SDL_EVENT_WINDOW_CLOSE_REQUESTED, kTimestampNanoseconds, kUnknownBackendWindowId)).has_value());
    ASSERT_EQ(m_resolvers.windowRequests.size(), 1U);
    EXPECT_EQ(m_resolvers.windowRequests.back(), kUnknownBackendWindowId);

    constexpr std::uint32_t kInvalidMappedBackendWindowId{43};
    m_resolvers.windows.emplace(kInvalidMappedBackendWindowId, ponder::platform::WindowId::Invalid());
    EXPECT_FALSE(Translate(MakeWindowEvent(SDL_EVENT_WINDOW_CLOSE_REQUESTED, kTimestampNanoseconds, kInvalidMappedBackendWindowId)).has_value());

    m_resolvers.displayRequests.clear();
    EXPECT_FALSE(Translate(MakeDisplayEvent(SDL_EVENT_DISPLAY_MOVED, kTimestampNanoseconds, 0)).has_value());
    EXPECT_TRUE(m_resolvers.displayRequests.empty());

    constexpr std::uint32_t kUnknownBackendDisplayId{74};
    EXPECT_FALSE(Translate(MakeDisplayEvent(SDL_EVENT_DISPLAY_MOVED, kTimestampNanoseconds, kUnknownBackendDisplayId)).has_value());
    ASSERT_EQ(m_resolvers.displayRequests.size(), 1U);
    ...
    m_resolvers.displays.emplace(kInvalidMappedBackendDisplayId, ponder::platform::DisplayId::Invalid());
    EXPECT_FALSE(Translate(MakeDisplayEvent(SDL_EVENT_DISPLAY_MOVED, kTimestampNanoseconds, kInvalidMappedBackendDisplayId)).has_value());
...
TEST_F(SdlEventTranslatorTests, OwnsTranslatedDataAfterTheSdlSourceIsOverwritten)
{
    ...
    std::optional<ponder::platform::PlatformEvent> translated = Translate(source);
    ASSERT_TRUE(translated.has_value());
    ...
    ExpectTranslatedEvent(translated,
                          ponder::platform::WindowMovedEvent{.timestamp = MakeTimestamp(), .windowId = kWindowId, .position = {-500, 275}});
}
} // namespace
