...
{
static_assert(
    !std::same_as<pond::platform::DialogRequestId, pond::platform::WindowId>);
static_assert(
    !std::same_as<pond::platform::DialogRequestId, pond::platform::DisplayId>);
static_assert(!std::convertible_to<std::uint64_t,
                                   pond::platform::DialogRequestId>);
static_assert(!std::convertible_to<pond::platform::DialogRequestId,
                                   std::uint64_t>);
...
TEST(DialogTypesTests, OwnDescriptorsAndOutcomes)
{
    ...
    EXPECT_EQ(openDesc.defaultLocation,
    ...
    const pond::platform::DialogSelection selection{
        .paths = {std::filesystem::path{"C:/tmp/water.sdf"}},
    ...
    const pond::platform::DialogOutcome cancellation{
        pond::platform::DialogCancellation{}};
    EXPECT_TRUE(
        std::holds_alternative<pond::platform::DialogCancellation>(cancellation));
}
} // namespace
