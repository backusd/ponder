...
static_assert(std::is_same_v<decltype(std::declval<pond::platform::Window&>().SetMouseGrab(true)),
                             pond::core::VoidResult>);
static_assert(
    std::is_same_v<decltype(std::declval<const pond::platform::Window&>().IsMouseGrabbed()), bool>);
static_assert(
    std::is_same_v<decltype(std::declval<pond::platform::Window&>().SetRelativeMouseMode(true)),
                   pond::core::VoidResult>);
static_assert(std::is_same_v<
              decltype(std::declval<const pond::platform::Window&>().IsRelativeMouseModeEnabled()),
              bool>);
static_assert(
    std::is_same_v<decltype(std::declval<const pond::platform::Window&>().GetNativeHandle()),
...
TEST(WindowDescTests, ProvidesStableDefaults)
{
    ...
    EXPECT_EQ(desc.graphicsCompatibility, pond::platform::WindowGraphicsCompatibility::Default);
}
...
TEST(WindowDescTests, OwnsConfiguredValues)
{
    ...
    EXPECT_EQ(desc.minimumLogicalSize, (pond::platform::LogicalSize{300, 200}));
    EXPECT_EQ(desc.graphicsCompatibility, pond::platform::WindowGraphicsCompatibility::Vulkan);
}
} // namespace
