...
void ExpectMatrixNear(pond::math::Matrix3x3 actual, pond::math::Matrix3x3 expected, float tolerance = 1.0e-5F)
{
...
void ExpectInverseFailure(pond::core::Result<pond::math::Matrix3x3> result, pond::math::MathErrorCode expectedCode)
{
...
{
    return pond::math::Matrix3x3{static_cast<float>(rows[0][0]), static_cast<float>(rows[0][1]), static_cast<float>(rows[0][2]),
                                 static_cast<float>(rows[1][0]), static_cast<float>(rows[1][1]), static_cast<float>(rows[1][2]),
                                 static_cast<float>(rows[2][0]), static_cast<float>(rows[2][1]), static_cast<float>(rows[2][2])};
}
...
TEST(Matrix3x3AlgebraTests, AppliesArithmeticAndScalarOperations)
{
    ...
    EXPECT_EQ(lhs + rhs, (pond::math::Matrix3x3{10.0F, 10.0F, 10.0F, 10.0F, 10.0F, 10.0F, 10.0F, 10.0F, 10.0F}));
    EXPECT_EQ(lhs - rhs, (pond::math::Matrix3x3{-8.0F, -6.0F, -4.0F, -2.0F, 0.0F, 2.0F, 4.0F, 6.0F, 8.0F}));
    EXPECT_EQ(lhs * 2.0F, (pond::math::Matrix3x3{2.0F, 4.0F, 6.0F, 8.0F, 10.0F, 12.0F, 14.0F, 16.0F, 18.0F}));
...
TEST(Matrix3x3AlgebraTests, TransformsVectorsWithIdentityScaleShearAndRotation)
{
    ...
    EXPECT_EQ((quarterTurnZ * pond::math::Vector3{1.0F, 0.0F, 0.0F}), (pond::math::Vector3{0.0F, 1.0F, 0.0F}));
}
...
TEST(Matrix3x3AlgebraTests, TransposesAndComputesDeterminants)
{
    ...
    EXPECT_EQ(pond::math::Transpose(matrix), (pond::math::Matrix3x3{1.0F, 0.0F, 5.0F, 2.0F, 1.0F, 6.0F, 3.0F, 4.0F, 0.0F}));
    EXPECT_EQ(pond::math::Determinant(pond::math::Matrix3x3::Identity()), 1.0F);
    EXPECT_EQ(pond::math::Determinant(matrix), 1.0F);
    EXPECT_EQ(pond::math::Determinant(pond::math::Matrix3x3{2.0F, 0.0F, 0.0F, 0.0F, 3.0F, 0.0F, 0.0F, 0.0F, 4.0F}), 24.0F);
}
...
TEST(Matrix3x3AlgebraTests, InvertsRepresentativeMatrices)
{
    ...
    const pond::math::Matrix3x3 matrix{1.0F, 2.0F, 3.0F, 0.0F, 1.0F, 4.0F, 5.0F, 6.0F, 0.0F};
    const pond::math::Matrix3x3 expectedInverse{-24.0F, 18.0F, 5.0F, 20.0F, -15.0F, -4.0F, -5.0F, 4.0F, 1.0F};
    const pond::math::Matrix3x3 quarterTurnZ{0.0F, -1.0F, 0.0F, 1.0F, 0.0F, 0.0F, 0.0F, 0.0F, 1.0F};

    auto inverse = pond::math::Inverse(matrix);
    ASSERT_TRUE(inverse.HasValue());
    EXPECT_EQ(inverse.GetValue(), expectedInverse);
    EXPECT_TRUE(pond::math::IsNear(matrix * inverse.GetValue(), pond::math::Matrix3x3::Identity(), tolerance));
    EXPECT_TRUE(pond::math::IsNear(inverse.GetValue() * matrix, pond::math::Matrix3x3::Identity(), tolerance));
    ...
    auto scaleInverse = pond::math::Inverse(pond::math::Matrix3x3{2.0F, 0.0F, 0.0F, 0.0F, 4.0F, 0.0F, 0.0F, 0.0F, 8.0F});
    ASSERT_TRUE(scaleInverse.HasValue());
    EXPECT_EQ(scaleInverse.GetValue(), (pond::math::Matrix3x3{0.5F, 0.0F, 0.0F, 0.0F, 0.25F, 0.0F, 0.0F, 0.0F, 0.125F}));
}

TEST(Matrix3x3AlgebraTests, RejectsSingularNonFiniteAndUnrepresentableInverses)
{
    ExpectInverseFailure(pond::math::Inverse(pond::math::Matrix3x3{1.0F, 2.0F, 3.0F, 2.0F, 4.0F, 6.0F, 7.0F, 8.0F, 9.0F}),
                         pond::math::MathErrorCode::SingularMatrix);

    ExpectInverseFailure(
        pond::math::Inverse(pond::math::Matrix3x3{1.0F, 0.0F, 0.0F, 0.0F, std::numeric_limits<float>::infinity(), 0.0F, 0.0F, 0.0F, 1.0F}),
        pond::math::MathErrorCode::NonFiniteInput);

    ExpectInverseFailure(
        pond::math::Inverse(pond::math::Matrix3x3{std::numeric_limits<float>::denorm_min(), 0.0F, 0.0F, 0.0F, 1.0F, 0.0F, 0.0F, 0.0F, 1.0F}),
        pond::math::MathErrorCode::SingularMatrix);
}
...
TEST(Matrix3x3AlgebraTests, SeededWellConditionedMatricesMatchDoubleReference)
{
    ...
    for (int iteration = 0; iteration < 32; ++iteration)
    {
        ...
        const float determinantTolerance = static_cast<float>(std::abs(determinant) * 1.0e-5 + 1.0e-4);
