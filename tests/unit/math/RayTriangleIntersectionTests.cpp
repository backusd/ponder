...
[[nodiscard]] std::optional<DoubleTriangleHit> IntersectDoubleOracle(const pond::math::Ray& ray, const pond::math::Triangle& triangle)
{
...
void ExpectTriangleHitNear(std::optional<pond::math::RayTriangleHit> hit, float expectedDistance, float expectedBarycentric0,
                           float expectedBarycentric1, float expectedBarycentric2, float tolerance = 1.0e-5F)
{
    ...
    EXPECT_NEAR(hit->GetBarycentric0() + hit->GetBarycentric1() + hit->GetBarycentric2(), 1.0F, tolerance);
}

[[nodiscard]] pond::math::Vector3 ReconstructPoint(const pond::math::Triangle& triangle, const pond::math::RayTriangleHit& hit)
{
    return triangle.GetVertex0() * hit.GetBarycentric0() + triangle.GetVertex1() * hit.GetBarycentric1() +
...
[[nodiscard]] bool IsOutsideUncertaintyRegion(const DoubleTriangleHit& hit) noexcept
{
    ...
    return hit.distance > kMinimumBoundarySeparation && hit.barycentric0 > kMinimumBoundarySeparation &&
           hit.barycentric1 > kMinimumBoundarySeparation && hit.barycentric2 > kMinimumBoundarySeparation;
}

TEST(RayTriangleIntersectionTests, ReturnsDistanceAndOrderedBarycentricsForFaceHits)
{
    const auto triangle =
        MakeTriangle(pond::math::Vector3{-1.0F, 0.0F, -5.0F}, pond::math::Vector3{1.0F, 0.0F, -5.0F}, pond::math::Vector3{0.0F, 1.0F, -5.0F});
    const auto ray = MakeRay(pond::math::Vector3{0.0F, 0.25F, 0.0F}, pond::math::Vector3{0.0F, 0.0F, -1.0F});

    const auto hit = pond::math::Intersect(ray, triangle);

    ExpectTriangleHitNear(hit, 5.0F, 0.375F, 0.375F, 0.25F);
    ExpectVectorNear(ReconstructPoint(triangle, hit.value()), ray.GetOrigin() + ray.GetDirection() * hit->GetDistance());
}

TEST(RayTriangleIntersectionTests, AcceptsBothTriangleWindings)
{
    const auto triangle =
        MakeTriangle(pond::math::Vector3{-1.0F, 0.0F, -5.0F}, pond::math::Vector3{0.0F, 1.0F, -5.0F}, pond::math::Vector3{1.0F, 0.0F, -5.0F});
    const auto ray = MakeRay(pond::math::Vector3{0.0F, 0.25F, 0.0F}, pond::math::Vector3{0.0F, 0.0F, -1.0F});
...
TEST(RayTriangleIntersectionTests, AcceptsEdgeVertexAndDistanceZeroHits)
{
    ...
    ExpectTriangleHitNear(pond::math::Intersect(MakeRay(pond::math::Vector3{0.0F, 0.0F, 0.0F}, pond::math::Vector3{0.0F, 0.0F, -1.0F}), triangle),
                          5.0F, 0.5F, 0.5F, 0.0F);
    ExpectTriangleHitNear(pond::math::Intersect(MakeRay(pond::math::Vector3{-1.0F, 0.0F, 0.0F}, pond::math::Vector3{0.0F, 0.0F, -1.0F}), triangle),
                          5.0F, 1.0F, 0.0F, 0.0F);
    ExpectTriangleHitNear(pond::math::Intersect(MakeRay(pond::math::Vector3{0.0F, 0.25F, -5.0F}, pond::math::Vector3{0.0F, 0.0F, -1.0F}), triangle),
...
TEST(RayTriangleIntersectionTests, RejectsMissesBehindOriginParallelRaysAndZeroAreaTriangles)
{
    const auto triangle =
        MakeTriangle(pond::math::Vector3{-1.0F, 0.0F, -5.0F}, pond::math::Vector3{1.0F, 0.0F, -5.0F}, pond::math::Vector3{0.0F, 1.0F, -5.0F});
    const auto zeroArea =
        MakeTriangle(pond::math::Vector3{0.0F, 0.0F, -5.0F}, pond::math::Vector3{1.0F, 0.0F, -5.0F}, pond::math::Vector3{2.0F, 0.0F, -5.0F});

    EXPECT_FALSE(pond::math::Intersect(MakeRay(pond::math::Vector3{2.0F, 2.0F, 0.0F}, pond::math::Vector3{0.0F, 0.0F, -1.0F}), triangle));
    EXPECT_FALSE(pond::math::Intersect(MakeRay(pond::math::Vector3{0.0F, 0.25F, -6.0F}, pond::math::Vector3{0.0F, 0.0F, -1.0F}), triangle));
    EXPECT_FALSE(pond::math::Intersect(MakeRay(pond::math::Vector3{0.0F, 0.25F, 0.0F}, pond::math::Vector3{1.0F, 0.0F, 0.0F}), triangle));
    EXPECT_FALSE(pond::math::Intersect(MakeRay(pond::math::Vector3{0.5F, 0.0F, 0.0F}, pond::math::Vector3{0.0F, 0.0F, -1.0F}), zeroArea));
}

TEST(RayTriangleIntersectionTests, RejectsNearParallelAndNearDegenerateUncertainHits)
{
    const auto nearParallelTriangle =
        MakeTriangle(pond::math::Vector3{0.0F, 0.0F, -5.0F}, pond::math::Vector3{1.0F, -1.0F, -4.0F}, pond::math::Vector3{1.0F, -1.0F, -7.0F});
    ...
    const auto nearParallelRay = MakeRay(nearParallelHitPoint - nearParallelDirection * 2.0F, nearParallelDirection);
    ...
    const auto nearDegenerateTriangle =
        MakeTriangle(pond::math::Vector3{0.0F, 0.0F, -5.0F}, pond::math::Vector3{1.0F, -1.0F, -5.0F}, pond::math::Vector3{1.0F, -0.999999F, -5.0F});
    const pond::math::Vector3 nearDegenerateHitPoint =
        nearDegenerateTriangle.GetVertex0() * 0.5F + nearDegenerateTriangle.GetVertex1() * 0.25F + nearDegenerateTriangle.GetVertex2() * 0.25F;
    const auto nearDegenerateRay = MakeRay(nearDegenerateHitPoint + pond::math::Vector3{0.0F, 0.0F, 2.0F}, pond::math::Vector3{0.0F, 0.0F, -1.0F});
...
{
    ExpectTriangleHitNear(pond::math::Intersect(MakeRay(pond::math::Vector3{0.25e-6F, 0.25e-6F, 0.0F}, pond::math::Vector3{0.0F, 0.0F, -1.0F}),
                                                MakeTriangle(pond::math::Vector3{0.0F, 0.0F, -1.0e-6F}, pond::math::Vector3{1.0e-6F, 0.0F, -1.0e-6F},
                                                             pond::math::Vector3{0.0F, 1.0e-6F, -1.0e-6F})),
                          1.0e-6F, 0.5F, 0.25F, 0.25F, 1.0e-9F);

    ExpectTriangleHitNear(pond::math::Intersect(MakeRay(pond::math::Vector3{2.5e15F, 2.5e15F, 0.0F}, pond::math::Vector3{0.0F, 0.0F, -1.0F}),
                                                MakeTriangle(pond::math::Vector3{0.0F, 0.0F, -1.0e16F}, pond::math::Vector3{1.0e16F, 0.0F, -1.0e16F},
...
TEST(RayTriangleIntersectionTests, SeededCasesAgreeWithDoublePrecisionOracle)
{
    ...
    {
        const auto triangle = MakeTriangle(
            pond::math::Vector3{coordinateDistribution(generator), coordinateDistribution(generator), coordinateDistribution(generator) - 30.0F},
            pond::math::Vector3{coordinateDistribution(generator), coordinateDistribution(generator), coordinateDistribution(generator) - 30.0F},
            pond::math::Vector3{coordinateDistribution(generator), coordinateDistribution(generator), coordinateDistribution(generator) - 30.0F});
        ...
        const auto ray = MakeRay(hitPoint + pond::math::Vector3{0.0F, 0.0F, distance}, pond::math::Vector3{0.0F, 0.0F, -1.0F});
