...
void ExpectHitNear(std::optional<pond::math::RayBoxHit> hit, float expectedEntry, float expectedExit, float tolerance = 1.0e-5F)
{
...
void ExpectPointNear(pond::math::Vector3 actual, pond::math::Vector3 expected, float tolerance = 1.0e-5F)
{
...
[[nodiscard]] bool UpdateDoubleOracleInterval(float origin, float direction, float minimum, float maximum, double& entry, double& exit)
{
    if (direction == 0.0F)
    {
    ...
    double nearDistance = (static_cast<double>(minimum) - static_cast<double>(origin)) / directionValue;
    double farDistance = (static_cast<double>(maximum) - static_cast<double>(origin)) / directionValue;
...
[[nodiscard]] std::optional<DoubleRayBoxHit> IntersectDoubleOracle(const pond::math::Ray& ray, const pond::math::AxisAlignedBox& box)
{
    ...
    if (!UpdateDoubleOracleInterval(origin.x, direction.x, minimum.x, maximum.x, entry, exit) ||
        ...
        !UpdateDoubleOracleInterval(origin.z, direction.z, minimum.z, maximum.z, entry, exit) || exit < 0.0)
    {
        return std::nullopt;
    }
...
{
    const auto ray = MakeRay(pond::math::Vector3{-3.0F, 0.0F, 0.0F}, pond::math::Vector3{1.0F, 0.0F, 0.0F});
    const auto box = MakeBox(pond::math::Vector3{-1.0F, -1.0F, -1.0F}, pond::math::Vector3{1.0F, 1.0F, 1.0F});

    const auto hit = pond::math::Intersect(ray, box);
    ExpectHitNear(hit, 2.0F, 4.0F);

    ExpectPointNear(ray.GetOrigin() + ray.GetDirection() * hit->GetEntryDistance(), pond::math::Vector3{-1.0F, 0.0F, 0.0F});
    ExpectPointNear(ray.GetOrigin() + ray.GetDirection() * hit->GetExitDistance(), pond::math::Vector3{1.0F, 0.0F, 0.0F});
}
...
TEST(CollisionIntersectionTests, ReturnsZeroEntryForInsideAndBoundaryOrigins)
{
    ...
    ExpectHitNear(pond::math::Intersect(MakeRay(pond::math::Vector3{0.0F, 0.0F, 0.0F}, pond::math::Vector3{1.0F, 0.0F, 0.0F}), box), 0.0F, 1.0F);
    ExpectHitNear(pond::math::Intersect(MakeRay(pond::math::Vector3{-1.0F, 0.0F, 0.0F}, pond::math::Vector3{1.0F, 0.0F, 0.0F}), box), 0.0F, 2.0F);
    ExpectHitNear(pond::math::Intersect(MakeRay(pond::math::Vector3{-1.0F, 0.0F, 0.0F}, pond::math::Vector3{-1.0F, 0.0F, 0.0F}), box), 0.0F, 0.0F);
}
...
TEST(CollisionIntersectionTests, RejectsMissesIncludingBehindOriginAndParallelOutsideSlabs)
{
    ...
    EXPECT_FALSE(pond::math::Intersect(MakeRay(pond::math::Vector3{3.0F, 0.0F, 0.0F}, pond::math::Vector3{1.0F, 0.0F, 0.0F}), box));
    EXPECT_FALSE(pond::math::Intersect(MakeRay(pond::math::Vector3{0.0F, 2.0F, 0.0F}, pond::math::Vector3{1.0F, 0.0F, 0.0F}), box));
    EXPECT_FALSE(pond::math::Intersect(MakeRay(pond::math::Vector3{-3.0F, 3.0F, 0.0F}, pond::math::Vector3{1.0F, 0.0F, 0.0F}), box));
}

TEST(CollisionIntersectionTests, PreservesZeroWidthIntervalsForTangencyAndPointBoxes)
{
    const auto box = MakeBox(pond::math::Vector3{0.0F, 0.0F, 0.0F}, pond::math::Vector3{1.0F, 1.0F, 1.0F});
    const auto tangentRay = MakeRay(pond::math::Vector3{-1.0F, 1.0F, 0.5F}, pond::math::Vector3{1.0F, -1.0F, 0.0F});
    ...
    ExpectHitNear(pond::math::Intersect(MakeRay(pond::math::Vector3{0.0F, 0.0F, 0.0F}, pond::math::Vector3{1.0F, 0.0F, 0.0F}),
...
{
    const auto flatBox = MakeBox(pond::math::Vector3{-1.0F, -1.0F, 0.0F}, pond::math::Vector3{1.0F, 1.0F, 0.0F});

    ExpectHitNear(pond::math::Intersect(MakeRay(pond::math::Vector3{0.0F, -2.0F, 0.0F}, pond::math::Vector3{0.0F, 1.0F, 0.0F}), flatBox), 1.0F, 3.0F);
    EXPECT_FALSE(pond::math::Intersect(MakeRay(pond::math::Vector3{0.0F, -2.0F, 1.0F}, pond::math::Vector3{0.0F, 1.0F, 0.0F}), flatBox));
}

TEST(CollisionIntersectionTests, HandlesVerySmallAndVeryLargeFiniteCases)
{
    ExpectHitNear(pond::math::Intersect(MakeRay(pond::math::Vector3{0.0F, 0.0F, 0.0F}, pond::math::Vector3{1.0F, 0.0F, 0.0F}),
                                        MakeBox(pond::math::Vector3{1.0e-6F, -1.0e-6F, -1.0e-6F}, pond::math::Vector3{2.0e-6F, 1.0e-6F, 1.0e-6F})),
                  1.0e-6F, 2.0e-6F, 1.0e-10F);

    ExpectHitNear(pond::math::Intersect(MakeRay(pond::math::Vector3{0.0F, 0.0F, 0.0F}, pond::math::Vector3{1.0F, 0.0F, 0.0F}),
                                        MakeBox(pond::math::Vector3{1.0e16F, -1.0F, -1.0F}, pond::math::Vector3{1.0001e16F, 1.0F, 1.0F})),
...
TEST(CollisionIntersectionTests, SeededCasesAgreeWithDoublePrecisionSlabOracle)
{
    ...
    {
        const pond::math::Vector3 center{centerDistribution(generator), centerDistribution(generator), centerDistribution(generator)};
        ...
        pond::math::Vector3 direction{directionDistribution(generator), directionDistribution(generator), directionDistribution(generator)};
        ...
        const auto ray =
            MakeRay(pond::math::Vector3{originDistribution(generator), originDistribution(generator), originDistribution(generator)}, direction);
