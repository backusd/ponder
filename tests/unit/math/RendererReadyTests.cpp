...
void ExpectVectorNear(pond::math::Vector3 actual, pond::math::Vector3 expected, float tolerance = kRendererTolerance)
{
...
{
    auto view = pond::math::Matrix4x4::LookAt(pond::math::Vector3{0.75F, 1.25F, 6.0F}, target, pond::math::Vector3{0.0F, 1.0F, 0.0F});
...
{
    auto projection = pond::math::Matrix4x4::Perspective(pond::math::ToRadians(pond::math::Degrees{60.0F}), 16.0F / 9.0F, 0.25F, 80.0F, depth);
...
{
    auto projection = pond::math::Matrix4x4::InfinitePerspective(pond::math::ToRadians(pond::math::Degrees{60.0F}), 16.0F / 9.0F, 0.25F,
                                                                 pond::math::ProjectionDepth::ReverseZ);
    EXPECT_TRUE(projection.HasValue());
    return projection.GetValue();
}

[[nodiscard]] pond::math::Triangle RequireTriangle(pond::math::Vector3 vertex0, pond::math::Vector3 vertex1, pond::math::Vector3 vertex2)
{
...
[[nodiscard]] pond::math::AxisAlignedBox RequireBounds(const std::array<pond::math::Vector3, 3>& points)
{
    pond::math::Vector3 minimum = pond::math::ComponentMin(pond::math::ComponentMin(points[0], points[1]), points[2]);
    pond::math::Vector3 maximum = pond::math::ComponentMax(pond::math::ComponentMax(points[0], points[1]), points[2]);
...
[[nodiscard]] pond::math::Ray RequirePickingRay(pond::math::Matrix4x4 viewProjection, pond::math::Viewport viewport, pond::math::Vector3 screenPoint,
                                                float nearDepth, float farDepth)
{
    auto clipToWorld = pond::math::Inverse(viewProjection);
    EXPECT_TRUE(clipToWorld.HasValue());
    auto nearPoint =
        pond::math::UnprojectFromClipToWorld(clipToWorld.GetValue(), viewport, pond::math::Vector3{screenPoint.x, screenPoint.y, nearDepth});
    auto farPoint =
        pond::math::UnprojectFromClipToWorld(clipToWorld.GetValue(), viewport, pond::math::Vector3{screenPoint.x, screenPoint.y, farDepth});
    EXPECT_TRUE(nearPoint.HasValue());
    EXPECT_TRUE(farPoint.HasValue());

    auto ray = pond::math::Ray::Create(nearPoint.GetValue(), farPoint.GetValue() - nearPoint.GetValue());
    EXPECT_TRUE(ray.HasValue());
    return ray.GetValue();
}

[[nodiscard]] pond::math::Vector3 ReconstructHitPoint(const pond::math::Triangle& triangle, const pond::math::RayTriangleHit& hit) noexcept
{
...
{
    const pond::math::Matrix4x4 world = pond::math::Matrix4x4::Translation(pond::math::Vector3{0.75F, -0.25F, -2.0F}) *
    ...
    const pond::math::Vector3 localHitPoint = localVertex0 * 0.25F + localVertex1 * 0.35F + localVertex2 * 0.4F;
    ...
    const pond::math::Triangle worldTriangle = RequireTriangle(worldVertex0, worldVertex1, worldVertex2);
    const pond::math::AxisAlignedBox worldBounds = RequireBounds(std::array{worldVertex0, worldVertex1, worldVertex2});
    ...
    const pond::math::Vector3 worldTangent0 = pond::math::TransformVector(world, localVertex1 - localVertex0);
    const pond::math::Vector3 worldTangent1 = pond::math::TransformVector(world, localVertex2 - localVertex0);
    ...
    auto worldRoundTrip = pond::math::Unproject(viewProjection, viewport, worldProjected.GetValue());
    ...
    auto outsideBounds = pond::math::AxisAlignedBox::Create(worldHitPoint + pond::math::Vector3{1000.0F, 0.0F, 0.0F},
                                                            worldHitPoint + pond::math::Vector3{1001.0F, 1.0F, 1.0F});
    ASSERT_TRUE(outsideBounds.HasValue());
    EXPECT_EQ(frustum->Classify(outsideBounds.GetValue()), pond::math::CollisionClassification::Disjoint);

    const pond::math::Ray pickingRay = RequirePickingRay(viewProjection, viewport, worldProjected.GetValue(), nearDepth, farDepth);
    const std::optional<pond::math::RayBoxHit> boxHit = pond::math::Intersect(pickingRay, worldBounds);
    const std::optional<pond::math::RayTriangleHit> triangleHit = pond::math::Intersect(pickingRay, worldTriangle);
    ...
    const pond::math::Vector3 reconstructedTrianglePoint = ReconstructHitPoint(worldTriangle, *triangleHit);
    const pond::math::Vector3 reconstructedRayPoint = pickingRay.GetOrigin() + pickingRay.GetDirection() * triangleHit->GetDistance();
