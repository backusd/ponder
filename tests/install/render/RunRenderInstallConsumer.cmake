...
ponder_normalize_safe_child_path(
    "${PONDER_INSTALL_PREFIX}" "the render install prefix" normalized_install_prefix)
ponder_normalize_safe_child_path(
    "${PONDER_CONSUMER_BINARY_DIR}" "the render consumer build directory"
...
function(ponder_require_clean_consumer_commands input_text label)
    ...
    foreach(blocked_fragment IN ITEMS
        ...
        if(NOT blocked_position EQUAL -1)
            message(FATAL_ERROR
                "An unrelated or private project dependency leaked into ${label}: "
                "${blocked_fragment}")
        endif()
    endforeach()
    ...
    if(normalized_text MATCHES "(^|[ ;\"])(/yu[^ ;\"]*|-include-pch)([ ;\"]|$)"
        ...
        message(FATAL_ERROR "The render consumer unexpectedly uses a precompiled header in ${label}.")
    endif()
endfunction()
...
foreach(public_header IN ITEMS Bootstrap RenderError)
    set(header_path "${PONDER_INSTALL_PREFIX}/include/ponder/render/${public_header}.hpp")
    if(NOT EXISTS "${header_path}")
        message(FATAL_ERROR "The installed package is missing render header: ${header_path}")
    endif()
endforeach()
...
if(NOT installed_package_contents MATCHES "ponder::render([^_a-z0-9]|$)")
    message(FATAL_ERROR "The installed package does not export ponder::render.")
endif()
...
execute_process(
    COMMAND "${CMAKE_COMMAND}" --build "${PONDER_CONSUMER_BINARY_DIR}"
            --target ponder_render_install_consumer ${config_args} --verbose
    RESULT_VARIABLE build_result
    OUTPUT_VARIABLE build_output
    ERROR_VARIABLE build_error)
if(NOT build_result EQUAL 0)
    message(FATAL_ERROR
        "Building the installed render consumer failed: ${build_result}\n"
        "${build_output}\n${build_error}")
endif()
set(build_log "${build_output}\n${build_error}")
ponder_require_clean_consumer_commands("${build_log}" "render consumer build commands")
...
if(EXISTS "${compile_commands_file}")
    ...
    ponder_require_clean_consumer_commands(
        "${compile_commands}" "render consumer compile_commands.json")
else()
    message(STATUS
