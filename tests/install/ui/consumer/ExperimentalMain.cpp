...
int main()
{
    ...
    if (pond::core::GetBuildInfo().GetProjectName() != "ponder" || pond::ui::GetLibraryName() != std::string_view{"ui"})
    {
        return 1;
    }

    if (ponder::platform::LaunchProcess({}).HasValue())
    {
        return 2;
    }
    ...
    if (metrics.HasValue() || metrics.GetError().GetCode() != pond::render::ToErrorCode(pond::render::RenderErrorCode::InvalidState))
    {
        return 4;
    }
    ...
    if (created.HasValue() || created.GetError().GetCode() != pond::render::ToErrorCode(pond::render::RenderErrorCode::InvalidState))
    {
        return 5;
    }

    pond::ui::experimental::RectangleRenderer emptyRenderer;
    if (emptyRenderer.IsValid() || pond::ui::experimental::kRectangleFacadeExperimentalNotice.empty())
    {
        return 6;
    }
